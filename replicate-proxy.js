...
app.post("/api/depth", async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(
      "https://api-inference.huggingface.co/models/depth-anything/Depth-Anything-V2-Large-hf",
      {
        method: "POST",
        headers: {
...
async function processWithHuggingFaceSpaces(image, res) {
  try {
    ...
    const spaceResponse = await fetch(
      "https://depth-anything-depth-anything-v2.hf.space/api/predict",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
app.post("/api/veo3-video", async (req, res) => {
  try {
    ...
    if (!apiKey) {
      return res
        .status(500)
        .json({ error: "Replicate API key not configured" });
    }
    ...
    while (
      ...
      console.log(
        `🎬 Video Status: ${result.status} (${attempts}/${maxAttempts})`,
      );
    }
    ...
    if (result.status === "succeeded") {
      ...
      res
        .status(500)
        .json({ error: result.error || "Video generation failed" });
    }
  } catch (error) {
...
app.listen(PORT, () => {
  console.log(
    `🚀 DepthAnything V2 + Replicate Proxy Server running on http://localhost:${PORT}`,
  );
  console.log(`🎯 DepthAnything V2 API: http://localhost:${PORT}/api/depth`);
  console.log(`🎬 Video API: http://localhost:${PORT}/api/veo3-video`);
  console.log(
    `🔑 HuggingFace Token: ${process.env.HUGGINGFACE_API_TOKEN || process.env.HF_API_KEY || process.env.HG_API_KEY ? "✅ Found" : "⚠️  Missing (will use free tier)"}`,
  );
  console.log(
    `🔑 Replicate Token: ${process.env.REPLICATE_API_TOKEN ? "✅ Found" : "❌ Missing"}`,
  );
  console.log(
    `🎯 DepthAnything V2: State-of-the-art depth estimation (10x better than MiDaS)`,
  );
});
