const { PrismaClient, UserRole, ServiceStatus, ServiceType, YardSize } = require('@prisma/client')
  ...
  try {
    ...
    const visit3 = await prisma.serviceVisit.create({
      data: {
        ...
        dogsServiced: 1,
        notes: 'One-time spring cleanup - heavy leaf cover, multiple waste piles'
      }
    })

    console.log('✅ Created service visits:', [visit1.id, visit2.id, visit3.id])
    ...
    await prisma.commission.create({
      data: {
        ...
        serviceVisitId: visit1.id,
        amount: 25.00, // 10% of $250 service
        status: 'PENDING'
      }
    })
