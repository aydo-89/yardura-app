...
export const WellnessHeader: React.FC<WellnessHeaderProps> = ({
  ...
  const totalColorSamples = wellnessData.trends.colorDonut.normal +
  ...
  if (totalColorSamples > 0) {
    ...
    const yellowPercent = Math.round((wellnessData.trends.colorDonut.yellow / totalColorSamples) * 100);
    const redPercent = Math.round((wellnessData.trends.colorDonut.red / totalColorSamples) * 100);
    const blackPercent = Math.round((wellnessData.trends.colorDonut.black / totalColorSamples) * 100);
  ...
  const totalConsistencySamples = wellnessData.trends.consistencyStack.reduce((sum, w) => sum + w.normal + w.soft + w.dry, 0);
              ...
              {latestCopy.cta && (
                ...
                >
                  <Button
                    ...
                    style={{
                      backgroundColor:
                        latestStatus === 'attention' ? wellnessTheme.red : wellnessTheme.teal,
                      borderRadius: wellnessTheme.radiusLg,
                    }}
                  >
                    <a
