...
export const FrequencyAnalysis: React.FC<FrequencyAnalysisProps> = ({ weeklyData, dogCount = 1 }) => {
  // Calculate frequency statistics (weekly data)
  const depositsPerWeek = weeklyData.map(week => week.deposits);
  const avgDeposits = depositsPerWeek.length > 0
  ...
  const consistency = variation <= 0.5 ? 'Very Consistent' : variation <= 1 ? 'Moderately Consistent' : 'Variable';
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center p-3 bg-white rounded-lg border">
                ...
                <div className="text-right">
                  ...
                  <span className="text-xs text-slate-500 block">
                    ({minDepositsPerDogPerWeek.toFixed(1)}-{maxDepositsPerDogPerWeek.toFixed(1)}/week)
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white rounded-lg border p-4">
                <div className="flex items-end justify-center gap-2 h-12 mb-2">
                  {weeklyData.slice(-7).map((week, i) => {
                    ...
                    const heightPercent = depositsPerDog > 0 ? (depositsPerDog / maxDepositsPerDog) * 100 : 0;
                    return (
                      <div
                        key={i}
            ...
            <div className={`p-4 rounded-lg border ${
              avgDepositsPerDogPerWeek >= 10 && avgDepositsPerDogPerWeek <= 18 ? 'bg-green-50 border-green-200' :
              'bg-yellow-50 border-yellow-200'
            }`}>
              <div className={`text-sm font-medium mb-2 ${
                avgDepositsPerDogPerWeek >= 10 && avgDepositsPerDogPerWeek <= 18 ? 'text-green-800' : 'text-yellow-800'
              }`}>
                Frequency Assessment
              </div>
              <div className={`text-sm ${
                avgDepositsPerDogPerWeek >= 10 && avgDepositsPerDogPerWeek <= 18 ? 'text-green-700' : 'text-yellow-700'
              }`}>
