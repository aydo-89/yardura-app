...
export const ContentSignals: React.FC<ContentSignalsProps> = ({ signals, weekly }) => {
  ...
  const hasData = signals.some(signal =>
    signal.series.some(point => point.y > 0) ||
    signal.series.length > 0
  );
  ...
  const handleImageRequest = (signalType: string, count: number) => {
    ...
    alert(`📸 Image request submitted for ${count} ${signalType} detection(s). A veterinarian will review the images and contact you within 24 hours.`);
  };
        ...
        {hasData ? (
          <>
            ...
            {signals.map((signal, index) => {
              ...
              const isCritical = (signal.key === 'parasites' || signal.key === 'foreign') && latestValue > 0;

              return (
                <div key={signal.key} className="p-4 bg-white rounded-lg border border-slate-200 min-h-[120px] flex flex-col">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      {config.icon ? (
                        <config.icon className={`w-4 h-4 flex-shrink-0 ${signal.key === 'parasites' ? 'text-red-500' : 'text-slate-500'}`} />
                      ) : (
                      ...
                      <div className="text-xs text-center text-slate-600">
                        {(() => {
                          ...
                          if (latestWeekSamples > 0 && latestValue > 0) {
                            const proportion = Math.round((latestValue / latestWeekSamples) * 100);
          ...
          <div className="text-center py-8 text-slate-500">
            ...
            <p className="text-sm">
              We'll detect mucous, greasy, parasites, and foreign objects that may indicate health issues.
            </p>
          </div>
        )}
      </div>
