...
export const ConsistencyAnalysis: React.FC<ConsistencyAnalysisProps> = ({
  ...
  const handleImageRequest = (consistencyType: string, count: number) => {
    ...
    alert(`📸 Image request submitted for ${count} ${consistencyType.toLowerCase()} consistency samples. A veterinarian will review the images and contact you within 24 hours.`);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {statsWithPercentages.map((stat) => {
            ...
            const isConcerning = (stat.label === 'Soft' && stat.pct > 20) ||
                                (stat.label === 'Hard' && stat.pct > 15);
            const isCritical = (stat.label === 'Soft' && stat.pct > 40) ||
            ...
            return (
              <Card key={stat.label} className={`text-center ${colors.bg} ${colors.border} border-2`}>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="dry" stackId="a" fill="#F59E0B" /> {/* Amber - matches card theme */}
                </BarChart>
              </ResponsiveContainer>
            </div>
