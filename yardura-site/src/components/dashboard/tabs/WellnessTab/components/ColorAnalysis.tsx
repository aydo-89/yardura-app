...
export const ColorAnalysis: React.FC<ColorAnalysisProps> = ({ colorStats, weekly }) => {
  ...
  const handleImageRequest = (color: string, count: number) => {
    ...
    alert(`📸 Image request submitted for ${count} ${color} detection(s). A veterinarian will review the images and contact you within 24 hours.`);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {colorEntries.map(([key, stat]) => {
            ...
            const actualPercentage = totalSamples > 0 ? Math.round((stat.count / totalSamples) * 100) : 0;
            ...
            return (
              ...
              >
                <div className="text-xl font-bold mb-1" style={{ color: config.color }}
                ...
                {shouldShowConcerning && (
                  <div className="mt-2 pt-2 border-t border-slate-200">
                    <div className={`text-xs font-medium flex items-center justify-center gap-1 mb-1 ${
        ...
        <div className="flex justify-center mb-6">
          <svg viewBox="0 0 140 140" className="w-44 h-44">
            {(() => {
              ...
              const totalSamples = colorEntries.reduce((sum, [, s]) => sum + s.count, 0);
              const normalPercent = normalEntry && totalSamples > 0 ? Math.round((normalEntry[1].count / totalSamples) * 100) : 0;
              const yellowPercent = yellowEntry && totalSamples > 0 ? Math.round((yellowEntry[1].count / totalSamples) * 100) : 0;
              const redPercent = redEntry && totalSamples > 0 ? Math.round((redEntry[1].count / totalSamples) * 100) : 0;
              const blackPercent = blackEntry && totalSamples > 0 ? Math.round((blackEntry[1].count / totalSamples) * 100) : 0;
              ...
              return (
                <>
                  ...
                  >
                    {Math.max(normalPercent, filteredRedPercent, filteredBlackPercent, yellowPercent)}%
                  </text>
                  <text
                    x={center}
                    y="75"
                  ...
                  >
                    {normalPercent >= Math.max(filteredRedPercent, filteredBlackPercent, yellowPercent) ? 'Normal' :
                     filteredRedPercent >= Math.max(normalPercent, filteredBlackPercent, yellowPercent) ? 'Red' :
                     filteredBlackPercent >= Math.max(normalPercent, filteredRedPercent, yellowPercent) ? 'Black' : 'Yellow'}
                  </text>
                </>
              );
            })()}
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-4 mb-6">
          ...
          <div className="grid grid-cols-8 gap-2">
            {weekly.slice(0, 8).map((week, i) => {
              ...
              const hasIssues = week.colors.red > 0 || week.colors.black > 0 || week.colors.yellow > 0;
              ...
              return (
                <div key={i} className="text-center">
                  ...
                  <div className="text-xs text-slate-500" title={`${totalDeposits} samples analyzed this week`}>
