...
export const useWellnessData = (
  ...
  return useMemo(() => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      let mockColors = { normal: 0, yellow: 0, red: 0, black: 0 };
      let mockConsistency = { normal: 0, soft: 0, dry: 0 };
      let mockDeposits = 0;
      ...
      if (useMockForThisWeek) {
        ...
        const weekDiff = Math.abs((weekStart.getTime() - sept7Date.getTime()) / (1000 * 60 * 60 * 24 * 7));

        // More specific check: if this week contains Sept 7 or is adjacent to it
        const weekEnd = new Date(weekStart);
        weekEnd.setDate(weekStart.getDate() + 6);
        const isSept7Week = (sept7Date >= weekStart && sept7Date <= weekEnd) ||
        ...
        const isSept7WeekDebug = weekStart.getMonth() === 8 && weekEnd.getMonth() === 8 &&
                                weekStart.getDate() <= 7 && weekEnd.getDate() >= 7;

        if (isSept7Week || weekDiff <= 1 || isSept7WeekDebug) { // Within 1 week of Sept 7 or contains Sept 7
      ...
      const colors = useMockForThisWeek ? mockColors : { normal: 0, yellow: 0, red: 0, black: 0 };
      if (!useMockForThisWeek) {
      ...
      const consistency = useMockForThisWeek ? mockConsistency : { normal: 0, soft: 0, dry: 0 };
      let weekSoftCount = 0;
      let hasMucous = false;
      let hasGreasy = false;
      let hasDry = false;
      ...
      if (!useMockForThisWeek) {
        weekReadings.forEach((r) => {
          ...
          if (Math.random() < 0.03) { // 3% chance for demo purposes
            hasForeign = true;
          }
        });
      } else {
        // Use mock data for consistency totals
        consistencyTotals.normal += consistency.normal;
        consistencyTotals.soft += consistency.soft;
        consistencyTotals.dry += consistency.dry;
        weekSoftCount = consistency.soft;

        // Generate mock signal detection based on consistency (higher probability for demo)
        if (consistency.soft > 0) {
          hasMucous = Math.random() < 0.7; // 70% of soft stools have mucous
          hasGreasy = Math.random() < 0.5; // 50% of soft stools are greasy
        }
        hasDry = consistency.dry > 0;
        hasParasites = Math.random() < 0.15; // 15% chance for demo (increased)
        hasForeign = Math.random() < 0.12; // 12% chance for demo (increased)
      }
      ...
      signalSparklines: [
        ...
        },
        {
          ...
          series: weekly.map((w, i) => {
            ...
            return {
              x: `Week ${8 - i}`,
              y: hasRealData ? Math.floor(w.consistency.soft * 0.3) : Math.floor(Math.random() * 2) + 1, // 1-2 occurrences per week (no zeros)
            };
          }),
        },
        {
          ...
          series: weekly.map((w, i) => {
            // Use the actual issues data if available, otherwise generate mock data
            const weekData = weekly.find(wk => wk.startISO === w.startISO);
            if (weekData && weekData.issues.some(issue => issue.includes('Mucous') || issue.includes('Greasy'))) {
              return {
                x: `Week ${8 - i}`,
            ...
            return {
              x: `Week ${8 - i}`,
              y: hasRealData ? (Math.random() < 0.1 ? Math.floor(Math.random() * 2) + 1 : Math.floor(Math.random() * 2)) : (Math.random() < 0.15 ? Math.floor(Math.random() * 2) + 1 : Math.floor(Math.random() * 2)), // More common for demo, some zeros allowed
            };
          }),
        },
        {
          ...
          series: weekly.map((w, i) => {
            ...
            return {
              x: `Week ${8 - i}`,
              y: hasRealData ? (Math.random() < 0.08 ? Math.floor(Math.random() * 2) + 1 : Math.floor(Math.random() * 2)) : (Math.random() < 0.12 ? Math.floor(Math.random() * 2) + 1 : Math.floor(Math.random() * 2)), // More common for demo, some zeros allowed
            };
          }),
        },
      ],
