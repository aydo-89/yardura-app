  ...
  try {
    ...
    Object.entries(envVars).forEach(([key, value]) => {
      if (key === 'POSTGRES_DATABASE_URL' || key === 'SHADOW_DATABASE_URL') {
        warn(`${key} configured`, !value, 'Optional - set for Postgres migration');
      } else {
    ...
    test('SQLite datasource configured', schemaContent.includes('datasource db'));
    test('Postgres datasource configured', schemaContent.includes('datasource pg'));
    test('Postgres URL configured', schemaContent.includes('POSTGRES_DATABASE_URL'));
    ...
    featureFlags.forEach(flag => {
      ...
      warn(`${flag} configured`, value === undefined, 'Optional - defaults to false');
    });
