...
export class QuoteService {
  ...
  static async validateQuoteData(quoteData: QuoteData): Promise<Record<string, string>> {
    ...
    if (quoteData.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(quoteData.email)) {
      errors.email = 'Please enter a valid email address';
    }

    if (quoteData.phone && !/^\+?[\d\s\-\(\)]{10,}$/.test(quoteData.phone.replace(/\s/g, ''))) {
  ...
  static async submitQuote(quoteData: QuoteData): Promise<{ success: boolean; quoteId?: string; error?: string }> {
    try {
