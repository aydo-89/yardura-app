...
import { getPlaceByCityState, getPlaceByCountyState, getZctasIntersectingPlace } from '@/lib/pmtiles';
import { clipZctasToPlace, scoreZctas, simplifyForRender, calculateCoverageStats, clipsToCoverage } from '@/lib/geo';
...
export async function POST(req: NextRequest) {
  try {
    ...
    debug('api', `Searching ZIP codes for ${isCountySearch ? 'county' : 'city'}: ${searchLabel}, ${state}`);
    ...
    debug('api', `Found place polygon with geometry type: ${place.geometry.type}`);

    // Step 2: Get ZCTAs that intersect with the place polygon
    const zctas = await getZctasIntersectingPlace(place);
    debug('api', `Found ${zctas.features.length} ZCTA features intersecting place`);
    ...
    debug('api', `Response: ${zips.length} ZIPs, ${coverageStats.coveragePercent}% coverage`);
    ...
    if (err.message?.includes('Place not found')) {
      ...
      errorMessage = 'Data source unavailable. Please ensure PMTiles are built and accessible.';
    }
