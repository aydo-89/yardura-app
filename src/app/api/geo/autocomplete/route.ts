...
function loadLocationOptions(): LocationOption[] {
  ...
  for (const [key, zipCount] of cityZipCounts.entries()) {
    ...
    if (zipCount >= 1) { // Only include cities with ZIP codes
      options.push({
        id: `city-${key}`,
  ...
  console.log(`Loaded ${options.length} city options (${cityZipCounts.size} cities)`);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.predictions && data.predictions.length > 0) {
            ...
            console.log(`Google Places returned ${googleOptions.length} options for "${query}"`);
            return NextResponse.json({ options: googleOptions });
          }
        }
      } catch (placesError) {
        console.warn('Google Places API failed, falling back to CSV:', placesError);
      }
    }
