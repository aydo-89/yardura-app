...
import type { YardSize as PricingYardSize, Frequency as PricingFrequency } from '@/lib/priceEstimator';
...
const asPricingYardSize = (value: string | null | undefined): PricingYardSize | null => {
  if (!value) return null;
  const normalized = value.toLowerCase();
  const allowed: PricingYardSize[] = ['small', 'medium', 'large', 'xl'];
  return allowed.includes(normalized as PricingYardSize) ? (normalized as PricingYardSize) : null;
};

const asPricingFrequency = (value: string | null | undefined): PricingFrequency | null => {
  if (!value) return null;
  const normalized = value.toLowerCase();
  const allowed: PricingFrequency[] = ['weekly', 'biweekly', 'twice-weekly', 'monthly', 'onetime'];
  return allowed.includes(normalized as PricingFrequency) ? (normalized as PricingFrequency) : null;
};
...
export async function POST(
  ...
  try {
    ...
    if (!pricing && lead.dogs && yardSizeForPricing && frequencyForPricing) {
      ...
      if (lead.zipCode) {
        try {
          zoneMultiplier = await getZoneMultiplierForZip(lead.zipCode, lead.orgId);
        } catch (error) {
      ...
      try {
        pricing = await calculatePrice({
          ...
          propertyType: lead.serviceType === 'commercial' ? 'commercial' : 'residential',
          lastCleanedBucket: lead.lastCleanedBucket ?? undefined,
          lastCleanedDate: lead.lastCleanedDate ? lead.lastCleanedDate.toISOString() : undefined,
    ...
    if (lead.sprayDeck) {
      addOnHighlights.push(`Deck Spray (${lead.sprayDeckMode || 'each visit'})`);
    }
    if (lead.divertMode && lead.divertMode !== 'none') {
      addOnHighlights.push(`Waste diversion (${lead.divertMode === 'takeaway' ? 'takeaway' : `${lead.divertMode}%`})`);
    }
    ...
    const baseSiteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.yardura.com';
      ...
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 640px; margin: 0 auto; color: #1f2937;">
        ...
        <div style="background: #ffffff; padding: 32px; border-radius: 0 0 18px 18px; box-shadow: 0 15px 30px rgba(15, 23, 42, 0.08);">
          ...
          <p style="font-size: 12px; color: #94a3b8; margin-top: 32px;">
            Quote Reference: ${lead.id} - Submitted ${lead.submittedAt.toLocaleString('en-US', {
              month: 'short',
              day: 'numeric',
              year: 'numeric',
            })}
...
-- The Yardura Team`;
    ...
    const sendResult = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || 'Yardura Quotes <quotes@yardura.com>',
      to: recipients,
    ...
    await prisma.lead.update({
      ...
      data: {
        ...
        estimatedPrice: typeof monthlyPrice === 'number' ? monthlyPrice : lead.estimatedPrice,
      },
    });

    return NextResponse.json({ ok: true, leadId: lead.id, emailId: sendResult.data?.id ?? null });
  } catch (error) {
