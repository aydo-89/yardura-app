...
export async function GET(request: NextRequest) {
  ...
  const limit = Number.isFinite(limitParam)
    ? Math.min(Math.max(Math.floor(limitParam), 1), MAX_CAPTURES)
    : 20;
...
export async function POST(request: NextRequest) {
  ...
  if (!resolvedDogId && resolvedSuspectedDogIds.length > 0) {
    ...
    if (invalid) {
      return NextResponse.json(
        { ok: false, error: "Unknown dog in suspected list." },
        { status: 422 },
      );
    }
  }
  ...
  try {
    ...
    const updated = await prisma.customerWellnessCapture.update({
      ...
      data: {
        ...
        analysisModel: process.env.WELLNESS_OWNER_CAPTURE_MODEL ?? "gpt-5-mini",
      },
    });
