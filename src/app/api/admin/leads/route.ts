...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!session?.user || !['ADMIN', 'OWNER', 'TECH', 'SALES_REP'].includes(role)) {
    ...
    if (territoryIdFilter) {
      where.territoryId = territoryIdFilter === 'NULL' ? null : territoryIdFilter;
    }
    ...
    const serializedLeads = leads.map((lead) => {
      ...
      if (lead.pricingBreakdown) {
        try {
          const raw = typeof lead.pricingBreakdown === 'string'
      ...
      return {
        ...
        specialInstructions: metadata?.specialRequests || lead.specialInstructions || null,
      };
    });
