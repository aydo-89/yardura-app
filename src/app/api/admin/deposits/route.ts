...
export async function POST(req: NextRequest) {
  const session = await getServerSession(authOptions as any) as { user?: { email?: string } } | null as { user?: { email?: string } } | null
  if (!isAdmin(session?.user?.email)) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  }

  const body = await req.json()
  const { customerId, visitId, timestamp, count, weight_grams, moisture_percent, c_color, c_content, c_consistency, notes } = body || {}
  if (!customerId || !count) {
