...
import { AlertCircle, Search, Loader2, X, MapPin, CheckCircle } from 'lucide-react';
import { Alert, AlertDescription } from '@/components/ui/alert';
import ZIPSearchMap, { ZIPSearchResult, ServiceAreaData } from '@/components/maps/ZIPSearchMap';
...
const saveSearchedCities = (cities: SearchedCity[], businessId: string) => {
  ...
  try {
    localStorage.setItem(`${businessId}-searched-cities`, JSON.stringify(cities));
  } catch (error) {
...
export default function AdminZipSearch() {
  ...
  const [serviceAreaData, setServiceAreaData] = useState<ServiceAreaData | null>(null);
  ...
  const handleLocationSearch = async (query: string) => {
    ...
    try {
      const response = await fetch(`/api/geo/autocomplete?q=${encodeURIComponent(query)}&limit=20`);
      if (response.ok) {
        const data = await response.json();
        setLocationOptions(data.options);
      }
  ...
  const saveZipStatuses = (statuses: ZipStatus, businessId: string) => {
    ...
    try {
      localStorage.setItem(`${businessId}-zip-statuses`, JSON.stringify(statuses));
    } catch (error) {
  ...
  useEffect(() => {
    ...
    console.log('Loading persisted data on mount:', {
      ...
      searchedCities: persistedSearchedCities.map(c => `${c.city}, ${c.state}`)
    });
  ...
  useEffect(() => {
    const addedZips = Object.values(zipStatuses).filter(status => status === 'added').length;
    if (addedZips > 0) {
        ...
        if (data.groups && data.groups.length > 0) {
          ...
          setZipStatuses(prevStatuses => {
            ...
            Object.keys(updatedStatuses).forEach(zip => {
              if (updatedStatuses[zip] === 'added' && !actualAddedZips.has(zip)) {
  ...
  const searchZips = async () => {
    ...
    try {
      console.log(`Searching for ZIP codes in: ${selectedLocation.city}, ${selectedLocation.state}`);
      ...
      if (response.ok) {
        ...
        const existingIndex = updatedSearchedCities.findIndex(
          city => city.city === newSearchedCity.city && city.state === newSearchedCity.state
        );
  ...
  const addZipToZone = async (zipCode: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        setError(`Failed to add ZIP ${zipCode}: ${error.error || 'Unknown error'}`);
  ...
  const removeZipFromZone = async (zipCode: string, zoneId: string = 'zone-urban-core') => {
    setZipStatuses(prev => ({ ...prev, [zipCode]: 'removing' }));

    try {
      const response = await fetch(`/api/admin/service-areas?businessId=yardura&zoneId=${zoneId}&zipCode=${zipCode}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        setError(`Failed to remove ZIP ${zipCode}: ${error.error || 'Unknown error'}`);
        setZipStatuses(prev => ({ ...prev, [zipCode]: 'error' }));
      }
    } catch (err) {
  ...
  const handleSuburbanCitySearch = async (cityName: string, state: string) => {
    try {
      ...
      if (response.ok) {
        const data: ZIPSearchResult = await response.json();
        console.log(`✅ Successfully found ${data.zips.length} ZIPs for suburban city ${cityName}`);
        ...
        console.warn(`⚠️ Failed to search suburban city ${cityName}: ${response.status} - ${errorText}`);
  ...
  return (
    <div className="container mx-auto p-6 pt-20 max-w-7xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">ZIP Code Search & Management</h1>
        <p className="text-muted-foreground">
          Search for cities to find ZIP codes and add them to your service areas.
        </p>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="flex gap-4 items-end">
              <div className="flex-1 relative" data-autocomplete>
                <Label htmlFor="location" className="text-sm font-medium">Search City</Label>
                <Input
                  id="location"
                  value={locationQuery}
                    ...
                    {locationOptions.map((option) => (
                      ...
                      >
                        <div className="flex-1">
                          <div className="font-medium text-gray-900">{properCapitalize(option.city)}, {option.state}</div>
                          <div className="text-xs text-gray-500 flex items-center gap-2">
                            <Badge variant="secondary">Search to see ZIPs</Badge>
                          </div>
                        </div>
                      </button>
                    ))}
          ...
          <Card className="h-fit">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg font-semibold">Service Areas</CardTitle>
                <div className="flex gap-2">
                  ...
                  {serviceAreaData?.stats.totalZips && serviceAreaData.stats.totalZips > 0 && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        if (window.confirm(`Remove all ${serviceAreaData.stats.totalZips} ZIP codes?`)) {
                          handleClearAll();
                        }
                      }}
              ...
              {serviceAreaData?.stats.totalZips && serviceAreaData.stats.totalZips > 0 && (
                <p className="text-sm text-gray-600">
                  {serviceAreaData.stats.totalZips} ZIP codes across {serviceAreaData.groups.length} locations
                </p>
              )}
            </CardHeader>
            <CardContent>
              <div className="space-y-4 max-h-[600px] overflow-y-auto">
                {serviceAreaData?.groups && serviceAreaData.groups.length > 0 ? (
                  serviceAreaData.groups.map((group) => (
                    <div key={`${group.city}-${group.state}`} className="border border-gray-200 rounded-lg p-3 bg-gray-50">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex-1">
                          <h4 className="font-semibold text-gray-900">
                            {properCapitalize(group.city)}, {group.state}
                          </h4>
                          <p className="text-xs text-gray-500">{group.zips.length} ZIP codes</p>
                          
                          {/* Per-city progress bar */}
                          {(() => {
                            // Find the searched city data for this group
                            const cityData = searchedCities.find(city => 
                              city.city.toLowerCase() === group.city.toLowerCase() && 
                              city.state === group.state
                            );
                            
                            if (cityData) {
                              const totalZips = cityData.searchResult.zips.length;
                              const addedZips = cityData.searchResult.zips.filter(zip => zipStatuses[zip] === 'added').length;
                              const addingZips = cityData.searchResult.zips.filter(zip => zipStatuses[zip] === 'adding').length;
                              const selectedZips = addedZips + addingZips;
                              const progressPercent = totalZips > 0 ? (selectedZips / totalZips) * 100 : 0;
                              
                              return (
                                <div className="space-y-1 mt-2">
                                  <div className="flex justify-between text-xs text-muted-foreground">
                                    <span>{selectedZips} of {totalZips} ZIPs selected</span>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            if (window.confirm(`Remove all ${group.zips.length} ZIP codes from ${cityName}, ${group.state}?`)) {
                              // Remove all ZIP codes from this city
                              group.zips.forEach(zip => handleZipToggle(zip, 'remove'));
                              
                              // Also remove the city boundary layers from the map
                              const mapInstance = (window as any).maplibre_search;
                              if (mapInstance) {
                                const searchId = `${group.city}-${group.state}`.toLowerCase().replace(/\s+/g, '-');
                                try {
                                  mapInstance.removeLayer(`place-boundary-${searchId}`);
                                  mapInstance.removeLayer(`zcta-polygons-${searchId}`);
                                  console.log(`Removed layers for ${cityName}, ${group.state}`);
                                } catch (e) {
                                  console.warn(`Failed to remove layers for ${cityName}, ${group.state}:`, e);
                                }
                              }
                            }
                          }}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {group.zips.map((zipCode) => (
                          <div key={zipCode} className="flex items-center justify-between p-2 bg-white rounded border border-gray-200 hover:border-gray-300 transition-colors">
                            <span className="font-mono text-sm font-medium text-gray-800">{zipCode}</span>
                            <Button
                              variant="ghost"
                              size="sm"
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm text-gray-500 mb-4">
                      Search for cities or counties above to add ZIP codes to your service areas.
                    </p>
                  </div>
                )}
              </div>
