    ...
    <div className="container py-8">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {alerts.map((a) => (
          <Card key={a.id}>
            <CardHeader>
              <CardTitle className="text-sm">{a.level} · {a.kind}</CardTitle>
            </CardHeader>
            <CardContent>
              ...
              {!a.acknowledged && (
                <form action={async () => {
                  'use server';
                  await fetch('/api/admin/alerts/ack', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: a.id }) });
                }}>
