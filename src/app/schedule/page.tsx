            ...
            <form
              action={async (formData) => {
                ...
                await fetch('/api/schedule/request', {
                  ...
                  body: JSON.stringify({ jobId: first.id, action, nextVisitAt }),
                });
              }}
            >
              <div className="flex flex-col sm:flex-row gap-3">
                <input type="datetime-local" name="nextVisitAt" className="border p-2 rounded" />
                <button className="px-3 py-2 border rounded" name="action" value="reschedule">
                  Request Reschedule
                </button>
                <button className="px-3 py-2 border rounded" name="action" value="skip">
                  Skip Next
                </button>
              </div>
            </form>
