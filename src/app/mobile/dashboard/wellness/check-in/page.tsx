...
import MobileWellnessCheckInForm from "@/components/dashboard/mobile/MobileWellnessCheckInForm";

export default async function MobileWellnessCheckInPage() {
  const session = await safeGetServerSession(authOptions as any);
  if (!session?.user?.email) {
    redirect("/signin?callbackUrl=/mobile/dashboard/wellness/check-in");
  }
  ...
  return (
    <div className="px-4 py-6 space-y-6">
      <header className="space-y-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white">Weekly check-in</h1>
          <p className="text-sm text-slate-400">
            Share any symptoms or notes from this week. This helps us connect
            your feedback with InsightScoop samples.
          </p>
        </div>
      </header>

      <MobileWellnessCheckInForm
        dogs={user.dogs}
        access={{
