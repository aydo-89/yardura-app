...
import MobileWellnessCapture from "@/components/dashboard/mobile/MobileWellnessCapture";

export default async function MobileWellnessCapturePage() {
  const session = await safeGetServerSession(authOptions as any);
  if (!session?.user?.email) {
    redirect("/signin?callbackUrl=/mobile/dashboard/wellness/capture");
  }
  ...
  return (
    <div className="px-4 py-6 space-y-6">
      <header className="space-y-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white">Stool scan</h1>
          <p className="text-sm text-slate-400">
            Capture a stool photo for instant, non-diagnostic guidance.
          </p>
        </div>
      </header>

      <MobileWellnessCapture dogs={user.dogs} access={accessSummary} privacy={privacy} />
    </div>
  );
}
