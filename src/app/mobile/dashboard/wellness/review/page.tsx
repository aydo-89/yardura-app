...
import MobileWellnessReviewExport from "@/components/dashboard/mobile/MobileWellnessReviewExport";

export default async function MobileWellnessReviewPage() {
  const session = await safeGetServerSession(authOptions as any);
  if (!session?.user?.email) {
    redirect("/signin?callbackUrl=/mobile/dashboard/wellness/review");
  }
  ...
  return (
    <div className="px-4 py-6 space-y-6">
      <header className="space-y-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white">Review & export</h1>
      ...
      <MobileWellnessReviewExport dogs={user.dogs} />
    </div>
  );
}
