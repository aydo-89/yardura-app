...
import MobileWellnessFoodLog from "@/components/dashboard/mobile/MobileWellnessFoodLog";

export default async function MobileWellnessFoodLogPage() {
  const session = await safeGetServerSession(authOptions as any);
  if (!session?.user?.email) {
    redirect("/signin?callbackUrl=/mobile/dashboard/wellness/food-log");
  }
  ...
  return (
    <div className="px-4 py-6 space-y-6">
      <header className="space-y-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white">Food log</h1>
          <p className="text-sm text-slate-400">
            Track diet changes and scan ingredients for common allergens.
          </p>
        </div>
      </header>

      <MobileWellnessFoodLog
        dogs={user.dogs}
        access={{
