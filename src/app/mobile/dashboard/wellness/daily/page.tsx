...
import MobileWellnessDailyCheckInForm from "@/components/dashboard/mobile/MobileWellnessDailyCheckInForm";

export default async function MobileWellnessDailyCheckInPage() {
  const session = await safeGetServerSession(authOptions as any);
  if (!session?.user?.email) {
    redirect("/signin?callbackUrl=/mobile/dashboard/wellness/daily");
  }
  ...
  const [accessSummary, lastCheckIn] = await Promise.all([
    getCustomerWellnessAccess({ customerId: customer.id, orgId: customer.orgId }),
    prisma.customerWellnessDailyCheckIn.findFirst({
  ...
  return (
    <div className="px-4 py-6 space-y-6">
      <header className="space-y-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white">Daily check-in</h1>
          <p className="text-sm text-slate-400">
            Quick 10-second tap flow to capture today&apos;s wellness.
          </p>
        </div>
      </header>

      <MobileWellnessDailyCheckInForm
        dogs={user.dogs}
        access={{
        ...
        lastCheckInAt={lastCheckIn?.loggedAt.toISOString() ?? null}
      />
    </div>
  );
}
