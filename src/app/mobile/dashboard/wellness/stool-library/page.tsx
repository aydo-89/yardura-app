...
import MobileStoolLibrary from "@/components/dashboard/mobile/MobileStoolLibrary";

export default async function MobileStoolLibraryPage() {
  const session = await safeGetServerSession(authOptions as any);
  if (!session?.user?.email) {
    redirect("/signin?callbackUrl=/mobile/dashboard/wellness/stool-library");
  }
  ...
  return (
    <div className="px-4 py-6 space-y-6">
      <header className="space-y-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white">Stool library</h1>
          <p className="text-sm text-slate-400">
            Quick visual comparisons for common stool scenarios.
          </p>
        </div>
      </header>

      <MobileStoolLibrary entries={STOOL_LIBRARY_ENTRIES} />
    </div>
  );
}
