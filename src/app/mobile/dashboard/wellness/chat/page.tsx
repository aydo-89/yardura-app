...
import MobileWellnessChat from "@/components/dashboard/mobile/MobileWellnessChat";

export default async function MobileWellnessChatPage() {
  const session = await safeGetServerSession(authOptions as any);
  if (!session?.user?.email) {
    redirect("/signin?callbackUrl=/mobile/dashboard/wellness/chat");
  }
      ...
      <header className="space-y-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">
            Symptom-aware guidance with safety guardrails and follow-up prompts.
          </p>
        </div>
      </header>

      <MobileWellnessChat dogs={user.dogs} access={accessSummary} />
    </div>
  );
}
