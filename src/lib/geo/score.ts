...
export function scoreZctas(
  ...
  debug('score', `Scoring ${clips.length} ZCTAs with threshold ${threshold}, centroid bias: ${biasCentroid}`);

  const scored = clips.map(clip => {
    const shouldKeep = clip.overlapRatio >= threshold ||
                      (biasCentroid && clip.centroidInside);

    if (shouldKeep) {
      debug('score', `Keeping ZIP ${clip.zip}: ratio=${clip.overlapRatio.toFixed(4)}, centroidInside=${clip.centroidInside}`);
    }
...
export function getAreaThreshold(): number {
  ...
  if (envThreshold) {
    ...
    console.warn(`Invalid ZIP_AREA_THRESHOLD: ${envThreshold}, using default ${DEFAULT_AREA_THRESHOLD}`);
  }
