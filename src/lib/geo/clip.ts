...
import { rewindSafe, unkinkSafe, validateAndClean, safeIntersects, fastDisjoint } from './geometry';
...
export function clipZctasToPlace(
  ...
  for (const zctaFeature of candidates) {
    try {
      ...
      if (!zip || zip.length !== 5) {
        debug('clip', `Invalid ZIP in feature: ${JSON.stringify(zctaFeature.properties)}`);
        continue;
      }
      ...
      const centroidInside = turf.booleanPointInPolygon(centroid, placeClean as any);

      // Attempt intersection
      let clippedFeature: GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>;
      let clipArea = 0;
      ...
      try {
        ...
        if (intersection && turf.area(intersection) > 0) {
          ...
          clippedFeature = rewindSafe(intersection) as GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>;
          clipArea = turf.area(clippedFeature);
        } else {
          // Use original as approximation
          clippedFeature = zctaClean as GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>;
          clipArea = fullArea;
        }
      } catch (intersectError) {
        debug('clip', `Intersection failed for ZIP ${zip}, using original:`, intersectError);
        clippedFeature = zctaClean as GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>;
        clipArea = fullArea;
      }
