...
export async function validateFormSubmission(
  ...
  if (formType === 'quote') {
    ...
    if (data.contact?.email) {
      const emailValidation = validateEmail(data.contact.email);
    ...
    if (data.contact?.phone) {
      const phoneValidation = validatePhone(data.contact.phone);
    ...
    if (data.contact?.name) {
      const nameValidation = validateName(data.contact.name);
