 ...
 */
export async function checkZipEligibility(zipCode: string, businessId: string = 'yardura'): Promise<ZipEligibilityResult> {
  try {
    ...
    if (isEligible && zone) {
      const estimatedDelivery = await getEstimatedDeliveryTime(zipCode, businessId);
      return {
        eligible: true,
        zone,
 ...
 */
export async function getZonePriceMultiplier(zipCode: string, businessId: string = 'yardura'): Promise<number> {
 ...
 */
export async function getEstimatedDeliveryTime(zipCode: string, businessId: string = 'yardura'): Promise<string> {
  const zone = await getZoneForZip(zipCode, businessId);
 ...
 */
export async function getAllServiceableZips(businessId: string = 'yardura'): Promise<string[]> {
  const config = await getBusinessConfig(businessId);
 ...
 */
export async function getZoneInfo(zipCode: string, businessId: string = 'yardura'): Promise<ServiceZone | null> {
 ...
 */
export async function addZipToService(zipCode: string, zoneId: string, businessId: string = 'yardura'): Promise<boolean> {
 ...
 */
export async function getAvailableZones(businessId: string = 'yardura'): Promise<ServiceZone[]> {
  const config = await getBusinessConfig(businessId);
  return config.serviceZones;
}
