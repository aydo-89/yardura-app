...
export const getSteps = (frequency?: string, isCommercial?: boolean): StepConfig[] => [
  {
    id: 'zip-check',
    title: 'Service Area',
...
export const getStepById = (stepId: string, steps: StepConfig[]): StepConfig | undefined => {
  return steps.find(step => step.id === stepId);
};

export const isStepCompleted = (stepId: string, currentStepIndex: number, steps: StepConfig[]): boolean => {
  const stepIndex = getStepIndex(stepId, steps);
  return stepIndex < currentStepIndex;
};

export const isStepCurrent = (stepId: string, currentStepIndex: number, steps: StepConfig[]): boolean => {
  const stepIndex = getStepIndex(stepId, steps);
  return stepIndex === currentStepIndex;
};

export const canAccessStep = (stepId: string, currentStepIndex: number, steps: StepConfig[]): boolean => {
  const stepIndex = getStepIndex(stepId, steps);
