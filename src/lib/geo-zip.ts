...
export type GeoPolygon = GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>;
...
export interface ComprehensiveZipResult {
  polygonFeatures: GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>;
  allZips: string[];
 ...
 */
export async function geocodePlaceToPolygon(city: string, state: string): Promise<GeoPolygon> {
  ...
  const candidate = (items as any[]).find(i =>
    i?.geojson &&
    (i.class === 'boundary' || i.type === 'administrative' ||
     i.type === 'city' || i.type === 'town' || i.type === 'village' ||
     i.type === 'suburb' || i.type === 'municipality')
  ) || (items as any[]).find(i =>
    i?.geojson && i.importance > 0.5 // High importance places
 ...
 */
export async function fetchZctasForGeometry(placePoly: GeoPolygon, searchType?: string, placeName?: string, stateName?: string): Promise<ComprehensiveZipResult> {
  debug('fetch', `Fetching ZCTAs for ${searchType || 'city'}: ${placeName || 'Unknown'}, ${stateName || 'Unknown'}`);
  ...
  debug('fetch', `Searching ZIPs within bbox: [${bbox[0]}, ${bbox[1]}, ${bbox[2]}, ${bbox[3]}]`);

  // Collect ZIPs from all real data sources
  const allZips = new Set<string>();
  const polygonFeatures: GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>[] = [];
  ...
  if (isCountySearch && placeName && stateName) {
    debug('fetch', `Using county search parameters: ${placeName} County, ${stateName}`);
    
    // For county searches, we need to get all ZIP codes in the county
    // Load the ZIP database to get all ZIPs for this county
    
    try {
      const zipDataPath = path.join(process.cwd(), 'src', 'lib', 'zip-city-data.csv');
      ...
      for (const line of lines) {
        ...
        const [stateFips, csvStateName, stateAbbr, zipcode, county, cityName] = line.split(',');
      ...
      debug('fetch', `Found ${countyZips.length} ZIP codes for ${placeName} County, ${stateName} from CSV`);
      ...
      if (countyZips.length > 0) {
        ...
        debug('fetch', `Fetching state ZCTA polygons: ${zctaUrl.split('/').pop()}`);
        ...
        if (zctaResp.ok) {
          ...
          const filtered: GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>[] = [];
          for (const f of stateGeo.features as any[]) {
            const propZip = normalizeZip(
              (f.properties?.ZCTA5CE10 || f.properties?.ZCTA5 || f.properties?.GEOID10 || f.properties?.GEOID || f.properties?.ZIP || f.properties?.zip || '').toString()
            );
            ...
            try {
              ...
              try {
                intersects = turf.booleanIntersects(cleanPlace as any, cleanZcta as any);
              } catch {
              ...
              try {
                ...
                if (process.env.NODE_ENV === 'development') {
                  console.debug(`Intersect failed for ZIP ${propZip}:`, intersectErr);
                }
              }
              if (clipped && turf.area(clipped) > 0) {
                (clipped as any).properties = { ...(clipped as any).properties, zip: propZip };
                ...
                if (turf.booleanIntersects(cleanPlace, cleanZcta)) {
                  (cleanZcta as any).properties = { ...(cleanZcta as any).properties, zip: propZip };
              ...
              try {
                if (turf.booleanIntersects(placePoly as any, f as any)) {
                  (f as any).properties = { ...(f as any).properties, zip: propZip };
          ...
          debug('fetch', `Prepared ${filtered.length} accurate ZCTA polygons for ${placeName} County, ${stateName}`);
          ...
          return {
            polygonFeatures: { type: 'FeatureCollection', features: filtered } as GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>,
            allZips: Array.from(allZips)
          };
        }
      }
    } catch (error) {
      warn('fetch', `County ZIP lookup failed, falling back to centroid approach: ${error}`);
    }
  }

  // For city searches: Use geometric intersection with real ZCTA data as primary method
  if (!isCountySearch && placeName && stateName) {
    debug('fetch', `Primary approach: geometric intersection for city ${placeName}, ${stateName}`);
    ...
    try {
      ...
      if (zctaResp.ok) {
        const stateGeo = await zctaResp.json();
        debug('fetch', `Loaded ${stateGeo.features?.length || 0} ZCTA features for ${stateName}`);
        
        // Find ZCTAs that intersect with the city polygon using geometric methods
        const intersectingZctas: GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>[] = [];
        
        for (const zcta of stateGeo.features || []) {
          const zipProp = zcta.properties?.ZCTA5CE10 || zcta.properties?.ZCTA5 || zcta.properties?.zip;
          ...
          try {
            ...
            const bboxIntersects = !(
              zctaBbox[2] < placeBbox[0] || zctaBbox[0] > placeBbox[2] ||
              zctaBbox[3] < placeBbox[1] || zctaBbox[1] > placeBbox[3]
            );
        ...
        debug('fetch', `Geometric intersection found ${allZips.size} valid ZIPs with ${intersectingZctas.length} polygons`);

        // If we found good data, we can use it as primary, but still collect comprehensive data for major metros
        if (allZips.size > 0) {
          debug('fetch', `Geometric intersection found ${allZips.size} ZIPs with polygons`);
          // For major metros, still collect broader ZIP data for suburbs grouping
          const isMajorMetroCheck = ['minneapolis', 'saint paul', 'chicago', 'detroit', 'atlanta', 'houston', 'dallas', 'phoenix', 'los angeles', 'new york', 'boston', 'philadelphia', 'washington'].includes((placeName || '').toLowerCase());
          if (!isMajorMetroCheck) {
            debug('fetch', 'Not a major metro - using only geometric intersection results');
            skipApiFallbacks = true;
          } else {
            debug('fetch', 'Major metro detected - will collect additional suburban ZIP data');
          }
        }
      } else {
      ...
      warn('fetch', `Geometric intersection failed, falling back to APIs: ${geoError}`);
    }
  }

  // Only use API fallbacks if geometric intersection didn't work
  if (!skipApiFallbacks) {
    debug('fetch', 'Geometric intersection failed or not attempted, using API fallbacks');
  ...
  if (!city || !state) {
    throw new Error('Could not determine city/state from polygon coordinates');
  }
  ...
  if (isCountySearch && placeName && stateName) {
    ...
    debug('fetch', `County search fallback for ${placeName} County, ${stateName}`);
  }
  ...
  try {
    ...
    try {
      const zipDbPath = join(process.cwd(), 'src/lib/comprehensive_zip_database.json');
    ...
    if (cityZips && cityZips.length > 0) {
      debug('fetch', `Found ${cityZips.length} ZIPs in comprehensive database for ${city}, ${state}`);
      return createFeaturesFromZips(cityZips, city, state, placeClean as GeoPolygon);
    }
      }
    } catch (error) {
      ...
      for (const zipUrl of zipUrls) {
        ...
        try {
          ...
          if (zipResp.ok) {
            ...
            debug('fetch', `Zipcodestack returned data with ${Object.keys(zipData.results || {}).length} result sets`);
            ...
            if (zipData.query && zipData.results) {
              ...
              for (const [zipCode, locations] of Object.entries(zipData.results)) {
                if (Array.isArray(locations) && locations.length > 0) {
                  locations.forEach((location: any) => {
                    // Filter by state if we have state info
                    if (state && location.state !== state && location.state_code !== state) {
            ...
            if (records.length > 0) {
              debug('fetch', `Zipcodestack returned ${records.length} ZIP records for ${city}, ${state}`);
              data = { records };
              break; // Success, stop trying other URLs
            } else {
              debug('fetch', 'No matching ZIP codes found in Zipcodestack response');
            }
          } else {
            const errorText = await zipResp.text();
            warn('fetch', `Zipcodestack API error: ${zipResp.status} ${zipResp.statusText}`);
            debug('fetch', `Error response: ${errorText.substring(0, 200)}...`);
          }
        } catch (error) {
  ...
  if (!data) {
    try {
      ...
      if (zipResp.ok) {
        const zipData = await zipResp.json();
        debug('fetch', `Zippopotam returned ${zipData.places?.length || 0} places for ${city}, ${state}`);

        if (zipData.places && zipData.places.length > 0) {
          debug('fetch', `Zippopotam returned ${zipData.places.length} ZIPs for ${city}, ${state}`);
          data = {
      ...
      if (ZIPCODESTACK_API_KEY) {
        ...
        try {
          ...
          if (nearbyResp.ok) {
            ...
            if (nearbyData.results && Object.keys(nearbyData.results).length > 0) {
              // Filter ZIPs that are within reasonable distance of the city
              const nearbyRecords: any[] = [];

              for (const [zipCode, locations] of Object.entries(nearbyData.results)) {
              ...
              if (nearbyRecords.length > 0) {
                debug('fetch', `Found ${nearbyRecords.length} nearby ZIPs within 10 miles of ${city}, ${state}`);
  ...
  if (!data || !Array.isArray(data.records) || data.records.length === 0) {
    throw new Error(`Unable to retrieve ZIP code data for ${city}, ${state}. All data sources are currently unavailable. Please try again later or contact support.`);
  }
  ...
  try {
    ...
    if (zctaResp.ok) {
      ...
      const candidateZips = new Set(
        (Array.isArray(data.records) ? data.records : []).map((r: any) => normalizeZip(r.fields?.zip || r.fields?.zipcode || r.fields?.zcta5 || r.fields?.zcta || ''))
      );
      ...
      debug('fetch', `Prepared ${filtered.length} accurate ZCTA polygons for ${city}, ${state}`);

      // Add ZIPs from API data to our comprehensive collection
      const apiZips = (Array.isArray(data.records) ? data.records : [])
        .map((r: any) => normalizeZip(r.fields?.zip || r.fields?.zipcode || r.fields?.zcta5 || r.fields?.zcta || ''))
        .filter(Boolean);
      ...
      debug('fetch', `Total ZIPs collected: ${allZips.size}, Polygons available: ${polygonFeatures.length}`);
    ...
    if (data?.records) {
      ...
      debug('fetch', `Added ${fallbackZips.length} ZIPs from fallback API data`);
    }
  } else {
    debug('fetch', 'Skipping API fallbacks - using geometric intersection results');
  }

  debug('fetch', `Final comprehensive collection: ${allZips.size} total ZIPs, ${polygonFeatures.length} with polygons`);
  ...
  const MAJOR_METRO_CITIES = [
    ...
    'new york', 'los angeles', 'chicago', 'dallas', 'houston', 'washington', 'philadelphia', 'miami', 'atlanta', 'phoenix',
    'boston', 'san francisco', 'detroit', 'riverside', 'minneapolis', 'san diego', 'tampa', 'denver', 'baltimore', 'st. louis',
    'kansas city', 'las vegas', 'cleveland', 'pittsburgh', 'portland', 'cincinnati', 'sacramento', 'orlando', 'san antonio', 'indianapolis',
    'columbus', 'charlotte', 'virginia beach', 'milwaukee', 'providence', 'jacksonville', 'memphis', 'oklahoma city', 'louisville', 'richmond',
    'new orleans', 'raleigh', 'salt lake city', 'buffalo', 'rochester', 'birmingham', 'tucson', 'fresno', 'tulsa', 'urban honolulu',
    // Add common variations and saint/st variations
    'saint paul', 'saint louis', 'saint petersburg', 'fort worth', 'fort lauderdale', 'long beach', 'oakland', 'mesa', 'colorado springs',
    'virginia beach', 'omaha', 'albuquerque', 'nashville', 'seattle', 'austin', 'el paso', 'bakersfield', 'fresno', 'sacramento'
  ];
  const searchCityName = placeName || 'Unknown';
  const isMajorMetro = MAJOR_METRO_CITIES.includes(searchCityName.toLowerCase());
  
  debug('fetch', `Checking if ${searchCityName} is major metro: ${isMajorMetro}, ZIP count: ${allZips.size}`);
  ...
  if (isMajorMetro && allZips.size > 20) {
    ...
    debug('fetch', `${searchCityName} is a major metro with ${allZips.size} ZIPs - grouping by actual cities`);
    zipsByCity = await groupZipsByPrimaryCity(Array.from(allZips), searchCityName, stateName || 'Unknown');
    ...
    suburbanCities = Object.fromEntries(
      Object.entries(zipsByCity).filter(([cityName]) => cityName !== mainCityName)
    );

    debug('fetch', `Metro grouping complete: ${mainCityZips.length} main city ZIPs, ${Object.keys(suburbanCities).length} suburban cities`);
  } else {
    // For smaller cities, don't group - all ZIPs belong to the search city
    debug('fetch', `${searchCityName} is not a major metro (${allZips.size} ZIPs) - keeping all ZIPs together`);
    const mainCityName = properCapitalize(searchCityName);
  ...
  return {
    polygonFeatures: { type: 'FeatureCollection', features: polygonFeatures } as GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>,
...
export function scoreZipsByOverlap(
  ...
  const uniqueResults = Array.from(byZip.values()).sort((a, b) => b.overlapRatio - a.overlapRatio);
 ...
 */
function createFeaturesFromZips(zipCodes: string[], city: string, state: string, placePoly: GeoPolygon): ComprehensiveZipResult {
  ...
  debug('fetch', `Returning ${zipCodes.length} ZIPs from comprehensive database (no synthetic polygons)`);

  return {
    polygonFeatures: { type: 'FeatureCollection', features: [] } as GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>,
    allZips: zipCodes
  };
}


async function fetchTargetedZctas(zips: string[], stateName: string): Promise<GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>> {
  const stateAbbrLower = toStateAbbrLower(stateName);
  const stateSlug = toStateSlug(stateName);
  const zctaUrl = `https://raw.githubusercontent.com/OpenDataDE/State-zip-code-GeoJSON/master/${stateAbbrLower}_${stateSlug}_zip_codes_geo.min.json`;
  ...
  try {
    ...
    const data = await resp.json() as GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>;
    const filtered = data.features.filter(feature => {
      const zipProp = (feature.properties?.ZCTA5CE10 ?? feature.properties?.ZCTA5 ?? '').toString();
 ...
 */
async function groupZipsByPrimaryCity(zips: string[], searchCity: string, searchState: string): Promise<{ [cityName: string]: string[] }> {
  debug('group', `Starting CSV-based city grouping for ${zips.length} ZIPs (same logic as Service Areas)...`);
  
  // Load ZIP data using the same approach as Service Areas
  const csvData = await fs.promises.readFile(path.join(process.cwd(), 'src/lib/zip-city-data.csv'), 'utf-8');
  const lines = csvData.split('\n').slice(1); // Skip header
  ...
  debug('group', `CSV-based grouping complete: ${Object.keys(zipsByCity).length} cities found`);
