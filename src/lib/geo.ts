...
export type GeoFeatureCollection = GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>;
...
export function clipZctasToPlace(
  ...
  for (const zctaFeature of zctaFC.features) {
    try {
      ...
      const zip = (zctaFeature.properties?.ZCTA5CE20 ||
      ...
      if (!zip || zip.length !== 5) {
        console.warn(`Invalid ZIP code in feature: ${JSON.stringify(zctaFeature.properties)}`);
        continue;
      }
      ...
      try {
        ...
        try {
          geometriesIntersect = turf.booleanIntersects(cleanZcta as any, cleanPlace as any);
        } catch {
        ...
        try {
          ...
          const intersection = turf.intersect(cleanZcta as any, cleanPlace as any);
 ...
 */
export function unionFeatures(features: GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>[]): GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon> | null {
  ...
  try {
    ...
    for (let i = 1; i < validFeatures.length; i++) {
      try {
        ...
        if (nextUnion && nextUnion.geometry && nextUnion.geometry.coordinates) {
          ...
          console.warn(`Union returned invalid result for feature ${i}, skipping`);
        }
      } catch (unionError) {
        console.warn(`Union failed for feature ${i}, skipping:`, unionError instanceof Error ? unionError.message : unionError);
    ...
    console.warn('Union operation failed, returning first valid feature as fallback:', error instanceof Error ? error.message : error);
...
function simplifyFeature(feature: GeoPolygon, toleranceMeters: number): GeoPolygon {
  try {
...
export function calculateCoverageStats(
  ...
  try {
    ...
    if (place && place.geometry && (place.geometry.type === 'Polygon' || place.geometry.type === 'MultiPolygon')) {
      placeArea = turf.area(place);
    } else {
      console.warn('Invalid place geometry for area calculation, using fallback');
...
export function validateAndRepairPolygon(polygon: GeoPolygon): GeoPolygon {
  try {
    ...
    if (cleaned.geometry.type === 'Polygon') {
      ...
      cleaned.geometry.coordinates = cleaned.geometry.coordinates.map((polyCoords: any) => {
 ...
 */
export function getCombinedBbox(features: GeoPolygon[]): [number, number, number, number] | null {
  if (features.length === 0) return null;

  try {
    const bboxes = features.map(f => turf.bbox(f) as [number, number, number, number]);
