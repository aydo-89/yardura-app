 ...
 */
export async function getPlaceByCountyState(county: string, state: string): Promise<GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>> {
  ...
  try {
    ...
    if (featureCollection?.features?.length > 0) {
      ...
      if (place.geometry?.type === 'Point') {
        ...
        console.log(`Creating county boundary around point for ${county} County, ${state} at [${lng}, ${lat}]`);
 ...
 */
export async function getPlaceByCityState(city: string, state: string): Promise<GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>> {
  ...
  try {
    ...
    if (featureCollection?.features?.length > 0) {
      ...
      if (place.geometry?.type === 'Point') {
        const [lng, lat] = place.geometry.coordinates;
        console.log(`Creating boundary around point for ${city}, ${state} at [${lng}, ${lat}]`);
        
        // Determine city size based on known major cities
        let size = 0.05; // Default size
        
        const majorCities = {
          'new york': 0.15, 'los angeles': 0.20, 'chicago': 0.18, 'houston': 0.20,
          'phoenix': 0.15, 'philadelphia': 0.12, 'san antonio': 0.18, 'san diego': 0.15,
          'dallas': 0.15, 'san jose': 0.10, 'austin': 0.12, 'jacksonville': 0.15,
          'fort worth': 0.12, 'columbus': 0.10, 'charlotte': 0.10, 'san francisco': 0.08,
          'indianapolis': 0.10, 'seattle': 0.10, 'denver': 0.12, 'washington': 0.12
        };
 ...
 */
export async function getZctasIntersectingPlace(place: GeoJSON.Feature): Promise<GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>> {
  ...
  console.log(`Getting real ZIP codes for ${searchType}: ${placeName}, ${stateName}`);
  ...
  try {
    ...
    if (searchType === 'county') {
      ...
      console.log(`Found ${zipCodes.length} real ZIP codes for ${placeName} County, ${stateName}`);
    } else {
      zipCodes = cities.get(key) || [];
      console.log(`Found ${zipCodes.length} real ZIP codes for ${placeName} city, ${stateName}`);
      
      // If no city match, try county with same name
      if (zipCodes.length === 0) {
        zipCodes = counties.get(key) || [];
        if (zipCodes.length > 0) {
          console.log(`No city match, found ${zipCodes.length} ZIP codes for ${placeName} County, ${stateName}`);
        }
      }
    }
    ...
    console.log(`Using ${zipCodes.length} real ZIP codes:`, zipCodes.slice(0, 10), zipCodes.length > 10 ? '...' : '');

    // Load real ZCTA polygons from OpenDataDE (US Census data) - the original working approach
    console.log('Loading real ZCTA polygons from US Census data for ZIP codes:', zipCodes);
    ...
    console.log(`Fetching real ZCTA data for state "${stateName}" -> abbr: "${stateAbbrUpper}" -> lower: "${stateAbbrLower}", name: "${stateNameSlug}"`);
    ...
    console.log(`Loaded ${stateGeo.features?.length || 0} real ZCTA features for ${stateName}`);
    ...
    for (const zcta of stateGeo.features || []) {
      const zipProp = zcta.properties?.ZCTA5CE10 || zcta.properties?.ZCTA5 || zcta.properties?.GEOID || zcta.properties?.zip;
      ...
      try {
        ...
        if (clippedFeature && turf.area(clippedFeature) > 0) {
          ...
          console.log(`Added original ZCTA for ZIP ${zipProp} (intersection failed)`);
        }
    ...
    console.log(`Prepared ${features.length} real ZCTA polygons from US Census data`);
 ...
 */
export async function getZctasInBbox(bbox: [number, number, number, number]): Promise<GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>> {
  try {
    ...
    const allFeatures: GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>[] = [];
        ...
        if (response.ok) {
          ...
          for (const feature of stateGeo.features || []) {
            try {
              ...
              const intersects = !(
                featureBbox[2] < bbox[0] || featureBbox[0] > bbox[2] ||
                featureBbox[3] < bbox[1] || featureBbox[1] > bbox[3]
              );
          ...
          console.log(`Found ${allFeatures.length} ZCTA features from ${stateAbbr}`);
        }
      } catch (error) {
 ...
 */
export async function getZctaFeature(zipCode: string): Promise<GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon> | null> {
  ...
  console.log('Real ZCTA lookup not yet implemented - returning null instead of synthetic data');
