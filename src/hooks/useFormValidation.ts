...
export function useZipCodeValidation(defaultZipCode?: string) {
  return useForm({
    resolver: zodResolver(z.object({
      zipCode: z.string().min(5, 'ZIP code must be 5 digits').max(10, 'ZIP code must be 5 or 9 digits').regex(/^\d{5}(-\d{4})?$/, 'Invalid ZIP code format'),
    })),
    defaultValues: {
...
export function useServiceTypeValidation(defaultServiceType?: 'residential' | 'commercial') {
  return useForm({
    resolver: zodResolver(z.object({
      serviceType: z.enum(['residential', 'commercial'], { message: 'Please select a service type' }).optional(),
    })),
    defaultValues: {
...
export function getFieldError(errors: Record<string, unknown>, fieldName: string): string {
