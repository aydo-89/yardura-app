...
export default function DashboardKpis() {
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: k.alertsOpen,
      icon: AlertTriangle,
      gradient: k.alertsOpen > 0 ? 'from-orange-500 to-red-500' : 'from-gray-500 to-slate-500',
      bgGradient:
        k.alertsOpen > 0 ? 'from-orange-500/10 to-red-500/10' : 'from-gray-500/10 to-slate-500/10',
      borderColor: k.alertsOpen > 0 ? 'border-orange-500/20' : 'border-gray-500/20',
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  {kpi.trendUp === true && <TrendingUp className="h-4 w-4 text-green-400" />}
                  {kpi.trendUp === false && <TrendingDown className="h-4 w-4 text-red-400" />}
                  {kpi.trendUp === null && <Minus className="h-4 w-4 text-slate-400" />}
                </div>
              </div>
              ...
              <div className="text-slate-300 text-sm font-medium mb-3">{kpi.title}</div>

              <div className="trend-indicator px-3 py-1 rounded-full text-xs font-medium inline-flex items-center gap-1">
                {kpi.trendUp === true && <TrendingUp className="h-3 w-3 text-green-400" />}
                {kpi.trendUp === false && <TrendingDown className="h-3 w-3 text-red-400" />}
                {kpi.trendUp === null && <Minus className="h-3 w-3 text-slate-400" />}
                <span
                  className={`${
                    kpi.trendUp === true
                      ? 'text-green-400'
