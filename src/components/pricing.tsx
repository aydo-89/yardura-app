...
function PricingCard({
  ...
  return (
    <Card
      className={getColorClasses(color, popular)}
    >
      {popular && (
      ...
      <CardContent className="space-y-6">
        <div className="text-center">
          <div className="text-gradient text-4xl font-black mb-1">{formatPrice(perVisitCents)}</div>
          <div className="text-sm text-slate-600 font-medium">per visit</div>
        </div>

        <div className={`text-center text-sm text-slate-600 rounded-xl p-3 border ${
          color === 'emerald' ? 'bg-emerald-50/70 border-emerald-200/50' :
          color === 'green' ? 'bg-green-50/70 border-green-200/50' :
          color === 'teal' ? 'bg-teal-50/70 border-teal-200/50' :
          'bg-slate-50/70 border-slate-200/50'
        }`}>
          <div className="font-semibold text-slate-900">{formatPrice(monthlyCents)}/month</div>
          <div className="text-xs font-medium">({getFrequencyDisplayName(frequency)} service)</div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50/50 transition-colors duration-200">
            <CheckCircle className="size-5 text-green-600 flex-shrink-0" />
            <span className="text-sm text-slate-700 font-medium">Health insights included</span>
          </div>
          {addOns.deodorize && (
            <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-teal-700-soft/30 transition-colors duration-200">
              <CheckCircle className="size-5 text-green-600 flex-shrink-0" />
              <span className="text-sm text-slate-700 font-medium">Deodorize & Sanitize (+$5)</span>
            </div>
          )}
          {addOns.litter && (
            <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-teal-700-soft/30 transition-colors duration-200">
              <CheckCircle className="size-5 text-green-600 flex-shrink-0" />
              <span className="text-sm text-slate-700 font-medium">Litter Box Service (+$5)</span>
            </div>
          )}
        </div>
        ...
        <Reveal delay={0.6}>
          <div className="mt-20 text-center">
            <div className="bg-white rounded-3xl p-10 max-w-4xl mx-auto shadow-floating border border-green-700/10">
              ...
              <Button size="lg" className="btn-cta-primary text-lg px-8 py-4" asChild>
                <a
                  href="/quote?businessId=yardura"
