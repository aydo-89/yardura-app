...
interface ZIPSearchMapProps {
  ...
  zipStatuses: { [zip: string]: 'available' | 'added' | 'adding' | 'removing' | 'error' };
...
const ZIPSearchMap = ({
  ...
  const layerRegistry = useRef<Map<string, {
    added: boolean;
    eventsBound: boolean;
  ...
  const toggleCityVisibility = useCallback((cityKey: string, visible: boolean) => {
    ...
    [placeLayerId, zctaLayerId].forEach(layerId => {
      ...
      if (layerInfo?.added) {
        try {
          ...
          mapInstance.map.setLayoutProperty(`${layerId}-stroke`, 'visibility', visible ? 'visible' : 'none');
  ...
  const updateZipColors = useCallback(() => {
    ...
    try {
      ...
      layerRegistry.current.forEach((layerInfo: any, layerId: string) => {
        if (layerInfo.added && layerInfo.featureCollection && layerId.startsWith('zcta-polygons-')) {
          try {
            // Update features with current ZIP statuses
            const updatedFeatures = layerInfo.featureCollection.features.map((feature: any) => ({
              ...feature,
              properties: {
                ...feature.properties,
  ...
  const handleSearchMapClick = useCallback((e: maplibregl.MapMouseEvent) => {
    const mapInstance = (window as any).maplibre_search;
    if (!mapInstance?.map) return;

    // Query all ZCTA layers (for cumulative searches)
    const layers = mapInstance.map.getStyle().layers || [];
    const zctaLayers = layers.filter((layer: any) => layer.id && layer.id.startsWith('zcta-polygons-')).map((layer: any) => layer.id);
    ...
    if (features && features.length > 0) {
      ...
      if (zip) {
        ...
        if (status === 'available') {
          ...
          console.log(`ZIP ${zip} not clickable - status: ${status}, available statuses:`, Object.keys(zipStatuses));
        }
      }
    }
  ...
  const setupHoverEffects = useCallback(() => {
    const mapInstance = (window as any).maplibre_search;
    if (!mapInstance?.map || !mapInstance.map.loaded() || !mapInstance.map.isStyleLoaded()) return;
    ...
    console.log(`Setting up hover effects for ${zctaLayers.length} ZCTA layers:`, zctaLayers);
  ...
  const initializeLayers = useCallback(() => {
    ...
    const searchId = `${searchResult.searchCriteria.city}-${searchResult.searchCriteria.state}`.toLowerCase().replace(/\s+/g, '-');
    ...
    try {
      ...
      console.log('ZCTA features count:', searchResult.map.includedZctas.features.length);
      
      // Add ZIP polygons with status-based coloring
      const zctaFeatures = searchResult.map.includedZctas.features.map(feature => ({
        ...feature,
        properties: {
          ...feature.properties,
          status: zipStatuses[feature.properties?.zip] || 'available'
      ...
      console.log('Adding ZCTA collection with', zctaFeatures.length, 'features');
      ...
      mapInstance.addGeoJsonLayer({
        ...
        fillColor: [
          ...
          [
            'match',
            ['get', 'status'],
            'added', '#dc2626', // Red hover for added (to remove)
            'adding', '#84cc16', // Same for adding
            'removing', '#f59e0b', // Same for removing
            'error', '#ef4444', // Same for error
            '#22c55e' // Green hover for available (to add)
          ],
          [
          'match',
          ['get', 'status'],
          'added', '#22c55e', // Green for added
          'adding', '#84cc16', // Light green for adding
          'removing', '#f59e0b', // Orange for removing
        ...
        strokeColor: [
          ...
          [
            'match',
            ['get', 'status'],
            'added', '#b91c1c', // Darker red hover for added
            'adding', '#65a30d', // Same for adding
            'removing', '#d97706', // Same for removing
            'error', '#dc2626', // Same for error
  ...
  useEffect(() => {
    ...
    const mapInstance = (window as any).maplibre_search;
    if (!mapInstance || !mapInstance.map || !mapInstance.map.loaded() || !mapInstance.map.isStyleLoaded()) {
      ...
      const retryTimeout = setTimeout(() => {
        ...
        if (newMapInstance && newMapInstance.map && newMapInstance.map.loaded() && newMapInstance.map.isStyleLoaded()) {
    ...
    console.log(`Map ready - restoring ${searchedCities.length} searched cities:`, searchedCities.map(c => `${c.city}, ${c.state}`));
    ...
    searchedCities.forEach(cityData => {
      ...
      if (cityData.searchResult?.map?.place && cityData.searchResult?.map?.includedZctas) {
        const searchId = `${cityData.city}-${cityData.state}`.toLowerCase().replace(/\s+/g, '-');
        const placeLayerId = `place-boundary-${searchId}`;
        const zctaLayerId = `zcta-polygons-${searchId}`;
        ...
        try {
          ...
          const zctaFeatures = cityData.searchResult.map.includedZctas.features.map(feature => ({
            ...feature,
            properties: {
              ...feature.properties,
              status: zipStatuses[feature.properties?.zip] || 'available',
          ...
          console.log(`Restored layers for ${cityData.city}, ${cityData.state}`);
        } catch (error) {
          console.warn(`Failed to restore layers for ${cityData.city}, ${cityData.state}:`, error);
        }
      }
    });
  ...
  }, [searchedCities, addedCities, zipStatuses, mapReady, updateZipColors, setupHoverEffects]);
  ...
  useEffect(() => {
    ...
    if (addedCities.has(cityKey)) {
      console.log(`City ${cityKey} already added, skipping layer initialization`);
      return;
    }
    ...
    checkAndInitialize();
  }, [searchResult?.searchCriteria?.city, searchResult?.searchCriteria?.state, addedCities]); // Only when search criteria changes
  ...
  const [serviceLayersInitialized, setServiceLayersInitialized] = useState(false);
  ...
  useEffect(() => {
    ...
    const updateServiceMap = () => {
      ...
      console.log('Updating service area map with', serviceAreaData.combined.features.length, 'features');
  ...
  const handleBulkAdd = () => {
    ...
    const availableZips = searchResult.zips.filter(zip => zipStatuses[zip] === 'available');
  ...
  const handleSuburbanZipToggle = async (zip: string, cityName: string, action: 'add' | 'remove') => {
    if (action === 'add') {
      // First auto-search for the suburban city's boundaries if not already searched
      if (onSuburbanCitySearch && !searchedCities.some(city => 
        city.city.toLowerCase() === cityName.toLowerCase() && 
        city.state === searchResult?.searchCriteria.state
      )) {
        console.log(`🔍 Auto-searching for suburban city FIRST: ${cityName}, ${searchResult?.searchCriteria.state}`);
        
        try {
          await onSuburbanCitySearch(cityName, searchResult?.searchCriteria.state || '');
          ...
          setTimeout(() => {
            console.log(`➕ Now adding ZIP ${zip} to ${cityName} after city boundaries loaded`);
            onZipToggle(zip, 'add');
          }, 2000);
        } catch (error) {
          console.error(`❌ Failed to search suburban city ${cityName}:`, error);
          // Fallback: just add the ZIP without city boundaries
          onZipToggle(zip, 'add');
        }
      } else {
        // City already searched, just add the ZIP
        console.log(`City ${cityName} already searched, adding ZIP ${zip} directly`);
        onZipToggle(zip, 'add');
      }
    } else {
      onZipToggle(zip, 'remove');
  ...
  const handleReverseZipLookup = async (zips: string[]) => {
    console.log(`Starting reverse lookup for ${zips.length} ZIPs to find their actual cities`);
    
    for (const zip of zips.slice(0, 5)) { // Limit to first 5 to avoid API spam
      try {
        ...
        if (response.ok) {
          ...
          if (data.places && data.places.length > 0) {
            ...
            if (actualCity.toLowerCase() !== searchResult?.searchCriteria.city.toLowerCase() && 
                onSuburbanCitySearch && 
                !searchedCities.some(city => 
                  city.city.toLowerCase() === actualCity.toLowerCase() && 
                  city.state === actualState
                )) {
              console.log(`Auto-searching for ZIP's actual city: ${actualCity}, ${actualState}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
              ...
              <div className="flex items-center gap-3 flex-1">
                <div>
                  <div className="text-xl font-bold text-slate-900">
                    {searchResult ? `${searchResult.searchCriteria.city}, ${searchResult.searchCriteria.state}` : 'ZIP Code Search Map'}
                  </div>
                  {searchResult && (
                    <div className="text-sm text-slate-600 font-medium">
                      {searchResult.count} ZIP codes found • {searchResult.coverageStats.coveragePercent}% coverage
                    </div>
                  )}
                </div>
                {searchResult && (
                  <Button
                    onClick={() => {
                      const cityKey = `${searchResult.searchCriteria.city}-${searchResult.searchCriteria.state}`.toLowerCase().replace(/\s+/g, '-');
                      const isVisible = layerRegistry.current.get(`place-boundary-${cityKey}`)?.visible ?? true;
                  ...
                  >
                    {layerRegistry.current.get(`place-boundary-${searchResult.searchCriteria.city.toLowerCase().replace(/\s+/g, '-')}-${searchResult.searchCriteria.state.toLowerCase()}`)?.visible ?? true ? 'Hide Layer' : 'Show Layer'}
                  </Button>
                )}
              </div>
            </CardTitle>
            ...
            <div className="p-6 bg-slate-50/50 border-b border-slate-200/60">
              <div className="flex flex-wrap gap-6 text-sm">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-700">City Boundary</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-5 h-5 rounded-lg bg-slate-200 border-2 border-slate-400"></div>
                  <span className="font-medium text-slate-700">Available ZIPs</span>
                </div>
                <div className="flex items-center gap-3">
            ...
            <div className="relative bg-slate-100 rounded-2xl overflow-hidden">
              ...
              {loading && (
                <div className="absolute inset-0 bg-white/90 backdrop-blur-sm flex items-center justify-center rounded-2xl">
                  <div className="flex flex-col items-center gap-3 p-6 bg-white rounded-2xl shadow-xl">
                    ...
                    <span className="text-sm font-medium text-slate-700">Loading ZIP data...</span>
                  </div>
                </div>
              )}
              ...
              {searchResult && (
                <div className="absolute top-4 right-4 flex flex-col gap-2">
                  ...
                  >
                    {layerRegistry.current.get(`place-boundary-${searchResult.searchCriteria.city.toLowerCase().replace(/\s+/g, '-')}-${searchResult.searchCriteria.state.toLowerCase()}`)?.visible ?? true ? '👁️ Hide' : '👁️‍🗨️ Show'}
                  </Button>
                </div>
              )}
            </div>
              ...
              <CardTitle className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-green-100 to-green-200 rounded-2xl">
                    <span className="text-lg font-bold text-green-600">{searchResult.count}</span>
                  </div>
                  <div>
                    <div className="text-xl font-bold text-slate-900">
                      ZIP Codes for {searchResult.searchCriteria.city}, {searchResult.searchCriteria.state}
                    </div>
                    <div className="text-sm text-slate-600 font-medium">
                      {searchResult.zips.filter(zip => zipStatuses[zip] === 'added').length} added • {searchResult.zips.filter(zip => zipStatuses[zip] === 'available').length} available
                    </div>
                  </div>
                </div>
                <Button
                  onClick={handleBulkAdd}
                  disabled={loading || !searchResult.zips.some(zip => zipStatuses[zip] === 'available')}
                  ...
                  <Badge
                    ...
                    className={`cursor-pointer transition-all duration-200 px-4 py-2 rounded-2xl font-semibold text-sm border-2 ${
                      zipStatuses[zip] === 'available'
                        ...
                        : zipStatuses[zip] === 'adding' || zipStatuses[zip] === 'removing'
        ...
        {searchResult?.zipsWithoutGeometry && searchResult.zipsWithoutGeometry.length > 0 && (
          <Card className="mt-4">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                <div>
                  <span>ZIPs Without Map Data for {searchResult.searchCriteria.city}, {searchResult.searchCriteria.state}</span>
                  <Button
                    onClick={() => handleReverseZipLookup(searchResult.zipsWithoutGeometry || [])}
                    size="sm"
                    variant="ghost"
                ...
                {(() => {
                  const availableZips = searchResult.zipsWithoutGeometry?.filter(zip => zipStatuses[zip] !== 'added') || [];
                  const addedZips = searchResult.zipsWithoutGeometry?.filter(zip => zipStatuses[zip] === 'added') || [];
                  ...
                  if (hasAdded && !hasAvailable) {
                    ...
                    return (
                      <Button
                        onClick={async () => {
                          console.log(`Removing all ${addedZips.length} ZIPs without map data`);
                          
                          // Remove all ZIPs with a small delay between each
                          for (let i = 0; i < addedZips.length; i++) {
                            const zip = addedZips[i];
                            console.log(`Removing ZIP ${zip} (${i + 1}/${addedZips.length})`);
                            onZipToggle(zip, 'remove');
                    ...
                    return (
                      <Button
                        onClick={async () => {
                          if (availableZips.length > 0) {
                            console.log(`Adding all ${availableZips.length} ZIPs without map data`);
                            
                            // Add all ZIPs with a small delay between each
                            for (let i = 0; i < availableZips.length; i++) {
                              const zip = availableZips[i];
                              console.log(`Adding ZIP ${zip} (${i + 1}/${availableZips.length})`);
                              onZipToggle(zip, 'add');
              ...
              <p className="text-sm text-muted-foreground">These ZIPs are associated with the current search but lack polygon data. Add/remove manually.</p>
            </CardHeader>
            <CardContent className="flex flex-wrap gap-2">
              {searchResult.zipsWithoutGeometry.map(zip => (
                <Badge
                  key={zip}
                  variant={zipStatuses[zip] === 'added' ? 'default' : 'outline'}
                  className="cursor-pointer"
                  onClick={() => onZipToggle(zip, zipStatuses[zip] === 'added' ? 'remove' : 'add')}
                >
                  {zip}
                  {zipStatuses[zip] === 'added' && <CheckCircle className="ml-1 h-3 w-3" />}
                </Badge>
              ))}
            </CardContent>
          </Card>
        ...
        {searchResult?.suburbanCities && Object.keys(searchResult.suburbanCities).length > 0 && (
          <Card className="mt-4">
          <CardHeader>
              <CardTitle>Associated Cities & Suburbs</CardTitle>
              <p className="text-sm text-muted-foreground">ZIPs from nearby cities found in your search. Adding these will automatically search for their map boundaries.</p>
          </CardHeader>
            <CardContent className="space-y-4">
              {Object.entries(searchResult.suburbanCities)
                .sort(([cityA], [cityB]) => cityA.localeCompare(cityB))
                .map(([cityName, cityZips]) => (
                <div key={cityName} className="border border-gray-200 rounded-lg p-3 bg-gray-50">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <div>
                        <h4 className="font-semibold text-gray-900">{cityName}</h4>
                        <span className="text-xs text-gray-500">{cityZips.length} ZIP codes</span>
                      </div>
                      <Button
                        onClick={() => {
                        ...
                        className="text-xs"
                      >
                        {layerRegistry.current.get(`place-boundary-${cityName.toLowerCase().replace(/\s+/g, '-')}`)?.visible ?? true ? 'Hide' : 'Show'}
                      </Button>
                    </div>
                    {(() => {
                      const availableZips = cityZips.filter(zip => zipStatuses[zip] !== 'added');
                      const addedZips = cityZips.filter(zip => zipStatuses[zip] === 'added');
                      const hasAvailable = availableZips.length > 0;
                      const hasAdded = addedZips.length > 0;
                      ...
                      if (hasAdded && !hasAvailable) {
                        ...
                        return (
                          <Button
                            onClick={async () => {
                              console.log(`Removing all ${addedZips.length} ZIPs from ${cityName}`);
                        ...
                        return (
                          <Button
                            onClick={async () => {
                              if (availableZips.length > 0) {
                                console.log(`Adding all ${availableZips.length} ZIPs from ${cityName}`);
                                ...
                                if (onSuburbanCitySearch && !searchedCities.some(city => 
                                  ...
                                  console.log(`Auto-searching for ${cityName} before adding all ZIPs`);
                                  try {
                                    await onSuburbanCitySearch(cityName, searchResult?.searchCriteria.state || '');
                                    // Wait for city to load
                                    await new Promise(resolve => setTimeout(resolve, 1500));
                                  } catch (error) {
                                    console.error(`Failed to search ${cityName}:`, error);
                                  }
                                }
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cityZips.map(zip => (
                      <Badge
                        ...
                        variant={zipStatuses[zip] === 'added' ? 'default' : 'outline'}
                        className="cursor-pointer"
                        onClick={() => handleSuburbanZipToggle(zip, cityName, zipStatuses[zip] === 'added' ? 'remove' : 'add')}
                      >
                        {zip}
                        {zipStatuses[zip] === 'added' && <CheckCircle className="ml-1 h-3 w-3" />}
                      </Badge>
