...
export const PropertyDetailsSchema = z.object({
  ...
  yardSize: z.enum(['small', 'medium', 'large', 'xlarge']).refine(val => val, {
...
export const ServiceFrequencySchema = z.object({
  frequency: z.enum(['weekly', 'bi-weekly', 'twice-weekly', 'monthly', 'one-time']).refine(val => val, {
...
export const CustomizationSchema = z.object({
  addOns: z.object({
    ...
    deodorizeMode: z.enum(['first-visit', 'each-visit', 'every-other', 'one-time']).optional(),
    sprayDeck: z.boolean().optional(),
    sprayDeckMode: z.enum(['first-visit', 'each-visit', 'every-other', 'one-time']).optional(),
    ...
    switch (stepId) {
      case 'zip-check':
        if (fieldName === 'zipCode') {
          schema = z.string().min(5, 'ZIP code must be 5 digits').max(5, 'ZIP code must be 5 digits').regex(/^\d{5}$/, 'Enter a valid 5-digit ZIP code');
        } else {
          return { valid: true };
        }
        break;
      ...
      case 'property-details':
        if (fieldName === 'serviceType') {
          ...
          schema = z.string().regex(/^\d{5}$/, 'Enter a valid 5-digit ZIP code');
        } else {
          return { valid: true };
        }
        break;
      case 'frequency':
        if (fieldName === 'frequency') {
          schema = z.enum(['weekly', 'bi-weekly', 'twice-weekly', 'monthly', 'one-time']);
        } else {
          return { valid: true };
        }
        break;
