...
import { stepServiceTypeSchema, StepServiceTypeData } from '@/lib/validations/quote';
...
export const StepServiceType: React.FC<StepProps> = ({
  ...
  const handleServiceTypeSelect = async (serviceType: 'residential' | 'commercial') => {
    setValue('serviceType', serviceType);
    updateQuoteData({ serviceType });
    track('service_type_selected', { serviceType });
    onNext?.();
  };
  const onSubmit = async (data: { serviceType?: 'residential' | 'commercial' }) => {
    if (data.serviceType) {
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              ...
              >
                <CardContent className="p-6 text-center">
                  ...
                  {quoteData.serviceType === 'residential' && (
                    <div className="mt-2 text-accent font-medium">✓ Selected</div>
                  )}
                </CardContent>
              </Card>
            </motion.div>
