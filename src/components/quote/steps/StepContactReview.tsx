...
export const StepContactReview: React.FC<StepProps> = ({
  ...
  onNext,
		...
		const load = async () => {
			try {
				...
				if (res.ok) {
					...
					setAvailableAddOns((config?.basePricing?.addOns || []).filter((a: AddOnConfig) => a.available));
				}
			} catch (e) {}
		};
		load();
	...
	const renderAddonLines = () => {
		...
		if (quoteData?.addOns?.deodorize) {
			...
			if (a) {
				...
				if (mode === 'each-visit') add('Deodorize & Sanitize (each visit)', `+$${base.toFixed(2)} / visit`);
				if (mode === 'every-other') add('Deodorize & Sanitize (every other)', `+$${(Math.round(a.priceCents / 2) / 100).toFixed(2)} / visit`);
				if (mode === 'first-visit' || mode === 'one-time') add('Deodorize & Sanitize (first visit only)', `+$${base.toFixed(2)} one-time`);
			}
		}

		if (quoteData?.addOns?.divertMode && quoteData.addOns.divertMode !== 'none') {
	...
	const areasSummary = useMemo(() => {
		...
		if (quoteData?.areasToClean?.other) areas.push(quoteData.areasToClean.other as unknown as string);
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Contact & Confirm
					...
					<p className="text-muted">Review your service details and enter your contact info</p>
        </CardHeader>
				<CardContent className="space-y-8">
					...
					<div>
						...
						<AddressAutocomplete
							...
							onChange={(value) => updateQuoteData({ address: value, addressValidated: false })}
							placeholder="Start typing your address"
						/>
						{!quoteData?.addressValidated && quoteData?.address && (
							<div className="mt-1 text-xs text-amber-600">Address not yet validated</div>
						)}
					</div>
					...
					<div className="bg-teal-700/5 border border-teal-700/20 rounded-lg p-6">
						<div className="flex items-center justify-between mb-4">
							<div className="text-lg font-medium text-teal-700/80">Service Summary</div>
							{visitsPerMonth && (
								<div className="text-sm text-muted-foreground">{visitsPerMonth.toFixed(2)} visits / month</div>
							)}
						</div>
						{estimatedPrice && (
							<div className="flex items-center justify-between">
								<div className="text-base text-gray-700">Billed monthly</div>
								<div className="font-bold text-teal-700 text-2xl">${(estimatedPrice as any).monthly || '—'}</div>
							</div>
						)}
					</div>
					...
					<div className="bg-white border rounded-lg p-6 space-y-4">
						<div className="text-lg font-medium text-gray-700 mb-4">Service Details</div>
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{quoteData?.dogs !== undefined && (
								<div className="flex justify-between text-base">
									<span className="text-gray-600">Dogs:</span>
									<span className="font-medium text-teal-700">{quoteData.dogs} dog{quoteData.dogs !== 1 ? 's' : ''}</span>
								</div>
							)}
							{quoteData?.yardSize && (
								<div className="flex justify-between text-base">
									<span className="text-gray-600">Property Type:</span>
									<span className="font-medium text-teal-700 capitalize">{quoteData.yardSize === 'xl' ? 'XL' : quoteData.yardSize}</span>
								</div>
							)}
							{quoteData?.frequency && (
								<div className="flex justify-between text-base">
									<span className="text-gray-600">Service Type:</span>
									<span className="font-medium text-teal-700 capitalize">{quoteData.frequency.replace('-', ' ')}</span>
								</div>
							)}
							{quoteData?.areasToClean && (
								<div className="flex justify-between text-base">
									<span className="text-gray-600">Areas to Clean:</span>
									<span className="font-medium text-teal-700">{areasSummary}</span>
								</div>
							)}
						</div>
						<div className="pt-4 border-t">
							<div className="text-lg font-medium text-gray-700 mb-3">Add-ons</div>
							{renderAddonLines()}
						</div>
						<div className="pt-4 border-t">
							<div className="text-lg font-medium text-gray-700 mb-1">Service Address</div>
							<div className="text-sm text-muted-foreground">{quoteData?.address || 'Not provided'}</div>
						</div>
					</div>
					...
					<div className="space-y-6">
						...
						<div>
							...
							<p className="text-sm text-muted mb-4">Select your preferred start date</p>
							...
							<div>
								<label className="block text-sm font-medium mb-2">Or select another date</label>
								<input
									type="date"
              ...
              <div>
                ...
                />
							...
							<div>
								<label className="block text-base font-medium mb-2">How did you hear about us?</label>
								<p className="text-sm text-muted mt-1 mb-3">Help us understand how you found Yardura</p>
								<select
									className="w-full p-3 border border-gray-300 rounded-lg bg-white"
									value={quoteData?.howDidYouHear || ''}
									onChange={(e) => updateQuoteData({ howDidYouHear: e.target.value })}
								>
