            ...
            {[
              {
                ...
                benefits: ['Fresh & clean every week', 'Prevent odor buildup', 'Regular schedule'],
                color: 'accent',
                recommended: true
              },
              {
                ...
                description: 'Perfect balance of cleanliness and cost-effectiveness',
                benefits: ['Every 2 weeks', 'Cost-effective option', 'Good for most yards'],
                color: 'blue',
                recommended: false
              },
              {
                ...
                benefits: ['2x per week service', 'Ultimate cleanliness', 'Heavy traffic areas'],
                color: 'purple',
                recommended: false
              },
              {
                ...
                benefits: ['Once per month', 'Budget-friendly', 'Light usage areas'],
                color: 'green',
                recommended: false
              },
            ].map((option) => (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className={`text-sm font-medium text-${option.color}-700 mb-2`}>{option.subtitle}</p>
                    <p className="text-sm text-gray-600 leading-relaxed">{option.description}</p>
                  </div>
                  {quoteData.frequency === option.value && (
                    <div className={`bg-${option.color}-500 text-white p-2 rounded-full shadow-md`}>
                ...
                <div className="space-y-2">
                  {option.benefits.map((benefit, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <div className={`w-1.5 h-1.5 bg-${option.color}-500 rounded-full flex-shrink-0`}></div>
                      <span className="text-sm text-gray-700">{benefit}</span>
                    </div>
                  ))}
                </div>

                <div className={`mt-4 inline-flex items-center gap-2 px-3 py-1 bg-${option.color}-100 text-${option.color}-800 text-xs font-medium rounded-full border border-${option.color}-200`}>
