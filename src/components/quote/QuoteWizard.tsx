      ...
      <Card className="shadow-sm">
        ...
        <CardContent className="space-y-3">
          ...
          {(() => {
            ...
            const isEmptyAddOns = !addOns ||
              (Object.keys(addOns).length === 0) ||
              (Object.keys(addOns).length === 1 && addOns.divertMode === 'none');
              ...
              return (
                <>
                  <div>
                    ...
                    <div className="text-xs text-gray-500 italic">None selected</div>
                  </div>
                  <div className="border-t border-gray-200 my-3" />
                </>
              );
            ...
            const hasDeodorize = addOns.deodorize === true || (addOns.deodorizeMode && addOns.deodorizeMode !== 'none');
            const hasSprayDeck = addOns.sprayDeck === true || (addOns.sprayDeckMode && addOns.sprayDeckMode !== 'none');
            const hasDivertMode = addOns.divertMode && addOns.divertMode !== 'none';
                ...
                <div>
                  ...
                  {hasDeodorize && (
                    <div className="flex justify-between items-center text-xs text-gray-600">
                      <span>
                        Deodorize & Sanitize
                        {addOns.deodorizeMode === 'first-visit' && ' (First visit only)'}
                        {addOns.deodorizeMode === 'each-visit' && ' (Each visit)'}
                        {addOns.deodorizeMode === 'every-other' && ' (Every other visit)'}
                        {addOns.deodorizeMode === 'one-time' && ' (One-time service)'}
                      </span>
                      <span>
                        {getAddonDisplay('deodorize', addOns.deodorizeMode).price}
                      </span>
                    </div>
                  )}
                  ...
                  {hasSprayDeck && (
                    <div className="flex justify-between items-center text-xs text-gray-600">
                      <span>
                        Spray Deck/Patio
                        {addOns.sprayDeckMode === 'first-visit' && ' (First visit only)'}
                        {addOns.sprayDeckMode === 'each-visit' && ' (Each visit)'}
                        {addOns.sprayDeckMode === 'every-other' && ' (Every other visit)'}
                        {addOns.sprayDeckMode === 'onetime' && ' (One-time service)'}
                      </span>
                      <span>
                        {getAddonDisplay('spray-deck', addOns.sprayDeckMode).price}
                      </span>
                    </div>
                  )}
                  ...
                  {hasDivertMode && (
                    <div className="flex justify-between items-center text-xs text-gray-600">
                      ...
                      <span>
                        {getAddonDisplay(`divert-${addOns.divertMode}`, 'selected').price}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {quoteData.addOns?.deodorizeMode === 'first-visit' && (
                    <div className="flex justify-between items-center text-xs text-gray-600">
                      ...
                      <span>{getAddonDisplay('deodorize', 'first-visit').price}</span>
                    </div>
                  )}
                  {quoteData.addOns?.sprayDeckMode === 'first-visit' && (
                    <div className="flex justify-between items-center text-xs text-gray-600">
                      <span>Spray Deck/Patio (first visit only)</span>
                      <span>{getAddonDisplay('spray-deck', 'first-visit').price}</span>
                    </div>
                  )}
                  {quoteData.addOns?.divertMode &&
                    ...
                    frequency === 'onetime' && (
                      <div className="flex justify-between items-center text-xs text-gray-600">
                        ...
                        <span>
                          {getAddonDisplay(`divert-${quoteData.addOns.divertMode}`, 'selected').price}
                        </span>
                      </div>
                    )}
                </div>
...
function QuoteWizardComponent() {
  ...
  const getBusinessOrgId = () => {
    ...
    if (sessionOrgId && ['ADMIN', 'OWNER', 'TECH', 'SALES_REP'].includes(userRole)) {
      return sessionOrgId;
    }
  ...
  console.log('Quote flow using orgId:', userOrgId, 'for user:', session?.user?.email || 'anonymous');
  ...
  const hasPropertyDetails = useMemo(() => {
    ...
    const hasLastCleanup = !!quoteData.deepCleanAssessment?.daysSinceLastCleanup;
    const hasAreas = !!quoteData.areasToClean && Object.values(quoteData.areasToClean).some((v) => !!v);
    return hasDogs && hasYard && hasLastCleanup && hasAreas;
  }, [quoteData.dogs, quoteData.yardSize, quoteData.deepCleanAssessment?.daysSinceLastCleanup, quoteData.areasToClean]);
  ...
  const handleSubmit = async () => {
    ...
    */
    ...
    try {
      ...
      if (quoteData.propertyType === 'commercial' || quoteData.serviceType === 'commercial') {
        ...
        >
          <div className="flex items-center justify-between mb-1">
            ...
            {currentStep >= 2 && _estimatedPrice && !_estimatedPrice.requiresCustomQuote && (
              ...
              >
                <div className="text-center">
                  <div className="text-sm font-bold text-green-700">${_estimatedPrice.oneTime}</div>
                  <div className="text-xs text-slate-500">One-time</div>
                </div>
                <div className="w-px h-6 bg-green-700/20"></div>
                <div className="text-center">
                  <div className="text-sm font-bold text-green-700">${_estimatedPrice.monthly}</div>
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur border-t border-slate-200 px-4 py-3">
          <div className="max-w-screen-xl mx-auto flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-4">
                {quoteData.frequency !== 'onetime' && (
                  <div className="text-sm">
                    ...
                    <div className="font-semibold">${_estimatedPrice.perVisit}</div>
                  </div>
                )}
                {quoteData.frequency !== 'onetime' ? (
                  <div className="text-sm text-right">
                    <div className="text-slate-500">Monthly</div>
                    <div className="font-semibold">${_estimatedPrice.monthly}</div>
                  </div>
                ) : (
                  <div className="text-sm text-right">
                    <div className="text-slate-500">One-time</div>
                    <div className="font-semibold">${_estimatedPrice.oneTime}</div>
                  </div>
                )}
              </div>
            ) : (
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <Button size="sm" onClick={currentStep === STEPS.length - 1 ? handleSubmit : handleNext} disabled={isSubmitting}>
          ...
          <>
            ...
            {isCommercial && (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-green-800">Commercial Property Service</p>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                {(isCommercial ? [
                  { value: 'small', label: 'Small Property', desc: 'Townhouse, condo, or small attached home', icon: '🏠' },
                  { value: 'medium', label: 'Medium Property', desc: 'Standard single-family home (under ½ acre)', icon: '🏡' },
                  { value: 'large', label: 'Large Property', desc: 'Spacious home or property (½ to 2 acres)', icon: '🏘️' },
                  { value: 'xl', label: 'Extra Large', desc: 'Estate or very large property (2+ acres)', icon: '🏰' }
                ] : [
                  { value: 'small', label: 'Small Property', desc: 'Townhouse, condo, or small attached home', icon: '🏠' },
                  { value: 'medium', label: 'Medium Property', desc: 'Standard single-family home (under ½ acre)', icon: '🏡' },
                  { value: 'large', label: 'Large Property', desc: 'Spacious home or property (½ to 2 acres)', icon: '🏘️' },
                  ...
                  >
                    ...
                    >
                      <CardContent className="p-4 text-center">
                        ...
                        <div className="text-xs text-muted leading-tight">{option.desc}</div>
                        {quoteData.yardSize === option.value && (
                          <div className="mt-2">
      ...
      {_estimatedPrice && quoteData.frequency && quoteData.frequency !== 'onetime' && (
        ...
        >
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <p className="text-sm font-medium text-green-700">Estimated Monthly Cost</p>
            <div className="w-2 h-2 bg-green-700 rounded-full"></div>
          </div>
          <p className="text-3xl font-bold text-green-700 mb-1">${_estimatedPrice.monthly}</p>
