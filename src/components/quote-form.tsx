...
import { calcOneTimeEstimate, calcPerVisitEstimate, type Frequency, type YardSize } from "@/lib/pricing";
...
const schema = z.object({
  ...
  const cityKey = Object.keys(SERVICE_ZIPS).find(k => k.toLowerCase() === data.city.trim().toLowerCase());
  ...
  if (!cityKey) {
    ...
    const hint = cityKey === "Minneapolis" || cityKey === "South Minneapolis" ?
...
export default function QuoteForm() {
  ...
  const { register, handleSubmit, watch, reset, formState: { errors, isSubmitting, isValid } } = useForm<FormData>({
    resolver: zodResolver(schema),
    defaultValues: { city: "Minneapolis", dogs: 1, yardSize: "medium", frequency: "weekly", deodorize: false, litter: false, dataOptIn: true }
  });
  ...
  useEffect(() => {
    if (frequency === "one-time") setEstimate(calcOneTimeEstimate(dogs, yardSize as YardSize, { deodorize }));
    else setEstimate(calcPerVisitEstimate(dogs, frequency as Frequency, yardSize as YardSize, { deodorize, litter }));
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6">
      ...
      <div className="grid md:grid-cols-5 gap-4">
        <div>
          ...
          <input type="number" min={1} max={8} {...register("dogs", { valueAsNumber: true })} className="border rounded-xl p-3 w-full" />
        </div>
        <div>
      ...
      <textarea rows={4} placeholder="Anything we should know? Gate code, dog names, etc." {...register("message")} className="border rounded-xl p-3" />

      <label className="inline-flex items-start gap-2 text-sm">
        <input type="checkbox" {...register("dataOptIn")} />
        <div className="flex-1">
          <span>
            I consent to Yardura collecting anonymized stool images during service to improve non‑diagnostic GI trend insights. I understand this is not medical advice and I may request deletion at any time.
          </span>
          <button
            type="button"
      ...
      <div className="flex items-center justify-between bg-brand-50 border rounded-xl p-4">
        <div>
          <div className="text-sm text-slate-600">Estimated {frequency === "one-time" ? "one-time" : "per visit"} price</div>
          <div className="text-3xl font-extrabold text-ink">${""}{estimate ?? 0}</div>
          <div className="text-xs text-slate-500">Final price confirmed after first visit / initial clean as needed.</div>
        </div>
        <div className="hidden md:block text-right">
          ...
          <button disabled={isSubmitting} className="px-5 py-3 rounded-xl bg-ink text-white shadow-soft hover:bg-brand-700">
            {isSubmitting ? "Processing..." : "Continue to Payment"}
          </button>
        </div>
        <button disabled={isSubmitting} className="md:hidden px-5 py-3 rounded-xl bg-ink text-white shadow-soft hover:bg-brand-700">
          {isSubmitting ? "Processing..." : "Continue to Payment"}
        </button>
      </div>
    </form>
