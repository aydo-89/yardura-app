...
const SECTION_VISUALS = {
  ...
  'tech-insights': 'Modern smartphone showing dog health app with colorful charts and data, clean Minneapolis backyard in background, technology meets pet care, professional service aesthetic',
  'reliable-service': 'Professional service provider in uniform with equipment in clean suburban Minneapolis yard, reliable weekly service, trustworthy and dependable aesthetic',
  'eco-friendly': 'Eco-friendly dog waste management process, green composting bins, sustainable practices, environmental responsibility, clean outdoor setting',
  
  // Services/How It Works section  
  'service-step1': 'Professional arriving at Minneapolis suburban home with service equipment, friendly service provider, clean uniform, reliable first impression',
  'service-step2': 'Thorough yard cleanup process in action, professional tools, attention to detail, clean suburban backyard transformation',
  'service-step3': 'Digital health insights being recorded on tablet, modern technology integration, data collection for pet wellness',
  
  // Eco section
  'environmental-impact': 'Small-scale composting operation in clean Minneapolis suburban backyard, eco-friendly composting bins, sustainable pet waste management, green community garden setting, professional composting facility, environmental responsibility, organic waste transformation to nutrient-rich compost',
  'waste-diversion': 'Composting process visualization, organic waste transformation, circular economy, environmental sustainability',
  
  // Why It Matters (simplified)
  'peace-of-mind': 'Happy pet owner receiving health notification on phone, peace of mind technology, early detection benefits, positive health monitoring',
  'vet-conversation': 'Pet owner having positive conversation with veterinarian, health data sharing, proactive pet care, professional consultation',
  
  // Pricing section
  'value-proposition': 'Clean yard transformation showing value of service, before and after comparison, investment in yard and pet health',
  
  // Testimonials/Social Proof
  'happy-customers': 'Real family photos with dogs in clean suburban Minneapolis yards, genuine smiles, authentic customer satisfaction, natural lighting, professional but warm atmosphere, diverse families enjoying their yards'
}

async function generateSectionVisual(preset, type = 'image', style = 'commercial') {
  ...
  try {
    const modelVersion = type === 'video' ? 'google/veo-3' : 'black-forest-labs/flux-schnell'
    ...
    if (result.status === 'succeeded') {
      ...
      console.log(`🎨 ${type.charAt(0).toUpperCase() + type.slice(1)} URL: ${result.output}`)
      console.log('')
      console.log(`💡 To download for section use:`)
      console.log(`   curl -o "./public/sections/${filename}" "${result.output}"`)
      console.log('')
      console.log('🎯 Integration suggestions:')
      console.log(`   • Add to ${preset} section as background or feature image`)
...
function showPresets() {
  ...
  Object.entries(categories).forEach(([category, presets]) => {
    ...
    presets.forEach(preset => {
      console.log(`  ${preset} - ${SECTION_VISUALS[preset].substring(0, 60)}...`)
    })
    console.log('')
  })
  ...
  console.log('  node create-section-visuals.js tech-insights image commercial')
  console.log('  node create-section-visuals.js service-step1 video lifestyle')
  console.log('  node create-section-visuals.js happy-customers image commercial')
}
